...
| `src/desktop/smoke/desktop-gui-smoke-runner.mjs` | Test-only 44-check GUI smoke sequence (785 lines) with factory injection of all shell dependencies. | main (test) | no | Injected showWindow, windows, shortcuts, electron APIs, smoke hooks, notification bridge. | low | Phase 2B.47 + REPO-1.1; returns runDesktopGuiSmoke + writeDesktopGuiSmokeResult. Compatibility barrel at `src/desktop/tray/desktop-gui-smoke-runner.mjs`. |
