...
| `src/service/capabilities/tools/document-renderer.mjs` | DOCX/PPTX/XLSX/PDF/HTML preview rendering. | service/artifact | no | docx/exceljs/pptx/pdf/html helpers, local Mermaid asset helper, shared SVG sanitizer. | high | Correct layer but heavy; worker may be needed for large docs. |
