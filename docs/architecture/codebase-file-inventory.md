...
| `src/service/action_tools/tools/index.mjs` | Tool aggregator: imports capability-owned tool families + owns remaining high-risk inline tools. | service/tools | partial | Aggregates from `src/service/capabilities/tools/`, connector tools, memory tools, vision tools, skill install tools, and artifact-path-helper. | medium | Live aggregator; remaining inline families include launch, write/edit/run, generate/render, file content/index/register/output, and capability draft/save. |
