...
| `src/service/action_tools/tools/index.mjs` | Tool aggregator: imports capability-owned tool families + owns remaining high-risk inline capability creator tools. | service/tools | partial | Aggregates from `src/service/capabilities/tools/`, connector tools, memory tools, vision tools, and skill install tools. | medium | Live aggregator; remaining inline family is capability draft/save. |
...
| `src/service/action_tools/tools/index.mjs` | Aggregator plus remaining capability creator inline tools. Low-risk, artifact, desktop, file, and render tool family moves are complete for the moved families only. |
