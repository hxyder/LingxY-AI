...
| `src/service/capabilities/tools/document-renderer.mjs` | DOCX/PPTX/XLSX/PDF/HTML preview rendering. | service/artifact | no | docx/exceljs/pptx/pdf/html helpers; local Mermaid/SVG helpers still old-owner until their own phase. | high | Correct layer but heavy; worker may be needed for large docs. |
