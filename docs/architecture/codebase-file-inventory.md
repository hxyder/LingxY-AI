...
| `src/service/action_tools/tools/index.mjs` | Tool aggregator and re-export surface for built-in action tools. | service/tools | no | Aggregates from `src/service/capabilities/tools/`, connector tools, memory tools, vision tools, and skill install tools. | medium | No inline tool implementation owners remain after CAP-5H. |
| `src/service/capabilities/tools/desktop-capture-gui-tools.mjs` | Screenshot capture plus Windows GUI find/click/type action tools. | service/tools/desktop | no | PowerShell screenshot/UIAutomation, artifact output helper, schemas/types. | high | Correct capability owner; GUI click/type remain confirmation-gated. |
| `src/service/capabilities/tools/desktop-launch-tools.mjs` | Local application launch/focus tool plus Windows/Python launcher disambiguation helpers. | service/tools/desktop | no | Python launcher script, PowerShell Start menu lookup, schemas/types. | high | Correct capability owner; preserves public launch ambiguity helper re-exports through the aggregator. |
| `src/service/capabilities/tools/file-content-tools.mjs` | File text extraction, folder text extraction, file-content search/indexing, artifact registration, and output path resolution tools. | service/tools/file-artifact | no | File ingest, local file collection, file read budget, evidence coverage, embedding namespace, manifest helpers, schemas/types. | high | Correct capability owner; `index_file_content` remains confirmation-gated and indexes only prior transcript reads. |
| `src/service/capabilities/tools/file-mutation-execution-tools.mjs` | File write/edit and short script execution tools. | service/tools/file-execution | no | Artifact path helper, file reversibility, document artifact helpers, schemas/types, subprocess. | high | Correct capability owner; preserves write/edit reversibility and run_script allowlist. |
| `src/service/capabilities/tools/document-artifact-helpers.mjs` | Shared document artifact kind inference, PDF/HTML generation helper, preview sidecar writing, renderer fallback, and generated-document checkpoint helper. | service/tools/artifact | no | Document renderer, Mermaid assets, SVG sanitizer, spreadsheet outline, file reversibility. | high | Shared by document-render tools and edit_file in file-mutation tools. |
| `src/service/capabilities/tools/document-render-tools.mjs` | Artifact-producing document, diagram, and SVG action tools. | service/tools/artifact | no | Document artifact helpers, Mermaid assets, SVG sanitizer, artifact path helper, schemas, registry result types. | high | Owns `generate_document`, `render_diagram`, and `render_svg`; aggregator re-exports only. |
| `src/service/capabilities/tools/capability-creator-tools.mjs` | Capability draft and save action tools for skills and MCP drafts. | service/tools/capability-management | no | Capability creator core, skill lifecycle, MCP drafts, schemas, registry result types. | high | Owns `draft_capability` and `save_capability_draft`; aggregator re-exports only. |
