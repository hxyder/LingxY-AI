...
| PMAT-015 Memory model and UX maturity | active | Align memory with mainstream assistant behavior: durable saved memory, reference chat/project history, and project knowledge are separate concepts. Routine task completion summaries are not user-facing pending durable memories by default. Durable memories remain user-controllable, editable, deletable, scoped, and reversible; project knowledge and conversation history are used as context without creating approval noise. | The default generated task-summary proposal mode is off. Explicit user-created memory and high-signal durable candidates can still use the approval queue; opt-in auto-save remains explicit. Existing approved memory injection remains scoped by global/project/conversation identifiers. The UI should collapse legacy routine episodic proposals or move them into activity/history tooling instead of asking the user to approve every completed task. |
