...
- Capability/tool owner cleanup: complete. `src/service/action_tools/tools/index.mjs`
  ...
  aggregators.
- Current green gate: `npm run check:fast` passed 141/141 after PMAT-010
  project UI cleanup was added, including 1127/1127 behavior
...
| PMAT-010 Project UI cleanup | complete | Make the Project surface as clean as Chat: compact project rail, central thread list with bottom composer, right context rail, and no dashboard-like stat cards or repeated explanatory text. | Project page uses `project-clean-layout`, compact project rows, compact workspace masthead, scrollable thread list, and restrained context rail. Verifiers check the clean layout and composer wiring while service ownership stays unchanged. |
