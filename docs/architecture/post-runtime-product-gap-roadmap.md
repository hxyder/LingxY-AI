...
| Runtime Labs entry | Role routing, reviewer loop, local trace taxonomy, sub-agent contracts | Keep Settings > Labs as the single user-facing place for experimental runtime gates. Only safe existing gates are user-toggleable; deferred network OTEL, multi-candidate voting, and automatic delegation show blocked reasons and next evidence gates. |
