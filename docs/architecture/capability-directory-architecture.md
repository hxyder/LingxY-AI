...
Status: inventory verified against the current repository on 2026-05-11 after CAP-5H capability creator tool migration.
...
| Shared capability helpers | `src/service/capabilities/tools/` | `open-with-default-handler.mjs`, `file-manifest-helpers.mjs`, `document-artifact-helpers.mjs` |
...
- Legacy paths for moved capability families must be deleted after active callers migrate unless a phase explicitly names a temporary migration window and guards it with a verifier.
