...
| --- | --- | --- |
| Action tools (built-in, remaining old owner) | `src/service/action_tools/tools/` | `index.mjs` (aggregator), `document-renderer.mjs`, `memory-tools.mjs`, `mermaid-assets.mjs`, `skill-install-tools.mjs`, `svg-sanitize.mjs` |
| Capability-owned tools | `src/service/capabilities/tools/` | `browser-web-tools.mjs`, `email-tools.mjs`, `file-read-tools.mjs`, `os-app-tools.mjs`, `scheduler-tools.mjs`, `vision-analyze.mjs` |
