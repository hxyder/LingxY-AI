...
| Voice and audio | Overlay voice input, voice attachments, note recording, Echo KWS, and audio transcription bridges stay wired while real microphone passes remain manual | `verify:audio-entrypoints`, `verify:local-http-surface`, `verify:notes-feature`, `verify:user-interaction-smoke` |
