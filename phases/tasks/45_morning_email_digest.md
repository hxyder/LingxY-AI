...
用户每天早上第一次启动 UCA 时（或早上 8-10 点时段启动），在 **5 分钟以内** 推送一条汇总："你昨天一共收到 N 封邮件，其中 M 封需要回复，按重要度依次是 ..."。支持聚合多个邮箱账户。
...
- 允许改动文件/目录：`src/service/email/digest.mjs`（新建）、`src/service/scheduler/engine.mjs`（加启动后一次性触发钩子）、`src/desktop/tray/electron-main.mjs`（加启动信号）、`phases/tasks/`
- 明确不做：多天汇总、周报、月报（看将来需求）

## 4. 交付产物

- **启动触发器**：
  - electron-main 在 `app.whenReady()` 之后往 service 发一条 `daily_digest_check` 信号
  - service 接到后判断：
    - 当前本地时间是否在 `[06:00, 12:00]` 范围内（可配置）
    - 今天是否已经推送过（用 `uca.lastDailyDigestDate` 本地状态文件判断）
    - 是否至少有一个 email 账户启用
  - 通过即触发 digest 生成
- **Digest 生成器** (`email/digest.mjs`)：
  - 拉取昨天 `[yesterday 00:00, today 00:00]` 的所有邮件（跨全部账户）
  - 按 "需要用户回复" / "被提及" / "纯通知" 分桶
  - 每桶用 fast executor 生成 ≤ 5 行要点
  - 总体拼成一条 `digest.md` artifact，同时生成一条短的 notification body
- **通知 + 浮窗打开**：
  - notification 窗口弹出 "早安，你昨天收到 N 封邮件..."
  - 点击 → 打开 overlay 并把 `digest.md` 载入为 context（用户可以继续追问"第 3 封是谁？"）
- **配置项**：在 Console Settings → Email Accounts 面板加：
  - "启用每日汇总" toggle
  - "推送时间范围" 两个时间输入
  - "跳过周末" toggle
...
- `node scripts/verify-email-morning-digest.mjs`（新建）：mock 昨天的邮件列表 + 端到端生成 digest artifact + notification 触发
