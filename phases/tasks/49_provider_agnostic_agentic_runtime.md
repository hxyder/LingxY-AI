...
- `scripts/verify-service-core.mjs` / `verify-action-tools.mjs` / `verify-kimi-runtime.mjs` —— count 更新 + 新断言
 - `src/service/ai/integrations/runtime.mjs` —— 统一 AI integrations（providers / code_cli / MCP / skills）热加载注册表
 - `src/service/ai/mcp/configured.mjs` / `src/service/ai/skills/builtin.mjs` / `src/service/ai/code_cli/configured.mjs` —— MCP/Skills/Code CLI runtime 配置解析
 - `src/desktop/renderer/console.js` / `console.html` —— Settings 新增 MCP / Skills / Code CLI 管理表单
 - `scripts/verify-ai-integrations.mjs` —— integrations 热加载验证（HTTP 写入 + 状态端点读取）
...
- 实际新增内容（commit 1 — provider-agnostic adapter + code_cli runtime resolution per task）：
  ...
  - **`src/service/core/http-server.mjs`**：
    ...
    - 新增 integrations 配置端点：
      - `GET /config/integrations`
      - `POST /config/mcp/servers` / `DELETE /config/mcp/servers/:id`
      - `POST /config/skills/registries` / `DELETE /config/skills/registries/:id`
      - `POST /config/code-cli/adapters` / `DELETE /config/code-cli/adapters/:id`
    - `/ai/mcp` 和 `/ai/skills` 返回状态 + skills 列表
...
- 验证结果（commit 1）：
  ...
  - `npm run check` —— 追加 `verify-ai-integrations`（MCP/Skills/Code CLI/Provider 动态配置验证）
...
- 实际新增内容（commit 2 — agentic planner + universal tool belt）：
  ...
  - **`src/service/core/service-bootstrap.mjs`**：
    ...
    - 引入 `createAIIntegrationRuntime`，统一 providers / code_cli / MCP / skills registry 的热加载入口
...
- 实际新增内容（commit 3 — code_cli JSON planning mode + provider visibility UI）：
  ...
  - **`src/desktop/renderer/console.js`** —— Console 任务详情面板的 provider 可见性：
    ...
    - 新增 MCP / Skills / Code CLI 管理区（新增 / 删除 / 刷新），调用 `/config/mcp/servers` / `/config/skills/registries` / `/config/code-cli/adapters`
  ...
  - **`scripts/verify-ai-integrations.mjs`（新脚本）**：
    - 启动本地 runtime，写入 MCP/Skills/Code CLI/Provider 配置
    - 断言 `/ai/providers` / `/ai/code-cli` / `/ai/mcp` / `/ai/skills` 立即反映新增配置
- 验证结果（commit 3）：
