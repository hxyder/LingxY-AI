 ...
 *      not duplicated).
 *   9. New chat clears consoleActiveConversation.
 *  10. Console + overlay use the EXACT same cache module
...
await it("console.js: Continue button binds to data-conversation-id and calls loadConsoleConversationFromBackend", () => {
  assert.match(consoleJs, /id="conversationsContinueBtn"/);
...
await it("console.js: New chat handler calls clearConsoleActiveConversation", () => {
  assert.match(consoleJs,
    /consoleChatNewBtn[\s\S]{0,400}clearConsoleActiveConversation\(\)/);
});
