import { existsSync, readFileSync } from "node:fs";

const requiredPaths = [
  "README.md",
  ".github/workflows/repo-baseline.yml",
  ".github/workflows/release-gate.yml",
  "index.cjs",
  "package.json",
  "docs/prd_v1.0.md",
  "docs/architecture/README.md",
  "AGENTS.md",
  "docs/architecture/agent-runtime-spine.md",
  "docs/architecture/electron-js-runtime-performance-plan.md",
  "docs/architecture/post-runtime-upgrade-roadmap.md",
  "docs/architecture/global-execution-efficiency-and-cleanup-plan.md",
  "docs/architecture/file-cleanup-evidence-pack.md",
  "docs/architecture/code-ownership-map.md",
  "docs/architecture/ipc-contract-inventory.md",
  "docs/architecture/http-route-ownership-inventory.md",
  "docs/architecture/tool-registry-inventory.md",
  "docs/architecture/artifact-surface-inventory.md",
  "docs/architecture/layer_overview.md",
  "docs/architecture/data_flow.md",
  "docs/architecture/process_topology.md",
  "docs/architecture/open_spikes.md",
  "docs/architecture/state_machines.md",
  "docs/protocols/context_packet.schema.json",
  "docs/protocols/task.schema.json",
  "docs/protocols/task_event.schema.json",
  "docs/protocols/artifact.schema.json",
  "docs/protocols/kimi_bridge_protocol.md",
  "docs/risks/risk_register_v1.md",
  "docs/action_tools/README.md",
  "docs/action_tools/tool_authoring_guide.md",
  "docs/action_tools/risk_assessment_template.md",
  "docs/browser_overlay/README.md",
  "docs/browser_overlay/show_rules_reference.md",
  "docs/browser_overlay/cross_site_test_matrix.md",
  "docs/pdf/README.md",
  "docs/pdf/pdf_test_corpus.md",
  "docs/pdf/ocr_engine_setup.md",
  "docs/privacy/README.md",
  "docs/privacy/privacy_policy.md",
  "docs/privacy/data_flow.md",
  "docs/privacy/pii_rules_reference.md",
  "docs/release/README.md",
  "docs/release/e2e_matrix.md",
  "docs/release/functional_acceptance_matrix.md",
  "docs/release/user_interaction_smoke_checklist.md",
  "docs/release/known_issues.md",
  "docs/release/rollback_plan.md",
  "docs/release/trial_release_notes_v0.1.0-trial.1.md",
  "docs/release/external_trial_checklist.md",
  "docs/release/trial_feedback_template.md",
  "docs/release/github_release_checklist.md",
  "tests/behavior/agent-loop-action-guidance.test.mjs",
  "tests/behavior/agent-loop-sequencing.test.mjs",
  "tests/behavior/agent-loop-confirmation-gate.test.mjs",
  "tests/behavior/agent-loop-error-budget.test.mjs",
  "tests/behavior/agent-loop-phase-gate.test.mjs",
  "tests/behavior/agent-loop-side-effect-gate.test.mjs",
  "tests/behavior/agent-loop-scheduled-fire-gate.test.mjs",
  "tests/behavior/agent-loop-saturation-gate.test.mjs",
  "tests/behavior/agent-loop-finalization.test.mjs",
  "tests/behavior/agent-loop-final-composer.test.mjs",
  "tests/behavior/agent-loop-launch-sequence.test.mjs",
  "tests/behavior/agent-loop-loop-policy.test.mjs",
  "tests/behavior/agent-loop-planner-formatting.test.mjs",
  "tests/behavior/agent-loop-planner-mode.test.mjs",
  "tests/behavior/agent-loop-conversation-messages.test.mjs",
  "tests/behavior/agent-loop-truthfulness-guard.test.mjs",
  "tests/behavior/agent-loop-tool-call-guards.test.mjs",
  "tests/behavior/agent-loop-tool-arg-repair.test.mjs",
  "tests/behavior/agent-loop-tool-surface.test.mjs",
  "tests/behavior/agentic-tool-result-controls.test.mjs",
  "tests/behavior/artifact-quality.test.mjs",
  "tests/behavior/artifact-fallback-policy.test.mjs",
  "tests/behavior/agentic-tool-surface.test.mjs",
  "tests/behavior/agentic-validator-transcript.test.mjs",
  "tests/behavior/agentic-finalization.test.mjs",
  "tests/behavior/agentic-tool-execution.test.mjs",
  "tests/behavior/agentic-user-message.test.mjs",
  "tests/behavior/account-connectors-external-call.test.mjs",
  "tests/behavior/code-cli-bridge-external-call.test.mjs",
  "tests/behavior/capability-checklist.test.mjs",
  "tests/behavior/capability-creator.test.mjs",
  "tests/behavior/capability-draft-tool.test.mjs",
  "tests/behavior/capability-save-tool.test.mjs",
  "tests/behavior/semantic-router-capability-management.test.mjs",
  "tests/behavior/connector-workflow-scheduled-authorization.test.mjs",
  "tests/behavior/connector-workflow-submission-boundary.test.mjs",
  "tests/behavior/document-diagram-components.test.mjs",
  "tests/behavior/svg-artifact-components.test.mjs",
  "tests/behavior/executor-loop-policy.test.mjs",
  "tests/behavior/external-call.test.mjs",
  "tests/behavior/failure-classifier.test.mjs",
  "tests/behavior/executor-selection.test.mjs",
  "tests/behavior/fast-executor-external-call.test.mjs",
  "tests/behavior/kimi-executor-external-call.test.mjs",
  "tests/behavior/local-mutation-guard.test.mjs",
  "tests/behavior/mermaid-local-assets.test.mjs",
  "tests/behavior/mcp-config-route.test.mjs",
  "tests/behavior/mcp-drafts-route.test.mjs",
  "tests/behavior/mcp-env-resolver.test.mjs",
  "tests/behavior/mcp-missing-config.test.mjs",
  "tests/behavior/mcp-install-detection.test.mjs",
  "tests/behavior/mcp-discovery-route.test.mjs",
  "tests/behavior/mcp-install-execution.test.mjs",
  "tests/behavior/mcp-install-route.test.mjs",
  "tests/behavior/mcp-install-sandbox.test.mjs",
  "tests/behavior/multi-modal-external-call.test.mjs",
  "tests/behavior/provider-catalog-models.test.mjs",
  "tests/behavior/provider-model-discovery.test.mjs",
  "tests/behavior/model-picker-view-model.test.mjs",
  "tests/behavior/desktop-gui-perf-smoke.test.mjs",
  "tests/behavior/desktop-actor.test.mjs",
  "tests/behavior/dock-geometry.test.mjs",
  "tests/behavior/local-file-fresh-read-contract.test.mjs",
  "tests/behavior/local-file-read-guidance.test.mjs",
  "tests/behavior/read-file-text-tool.test.mjs",
  "tests/behavior/requested-output-renderer.test.mjs",
  "tests/behavior/read-probe-cache.test.mjs",
  "tests/behavior/skill-lifecycle.test.mjs",
  "tests/behavior/task-runtime-event-emitter.test.mjs",
  "tests/behavior/task-runtime-event-log.test.mjs",
  "tests/behavior/task-runtime-composite.test.mjs",
  "tests/behavior/task-runtime-conversation-lifecycle.test.mjs",
  "tests/behavior/task-runtime-task-lifecycle.test.mjs",
  "tests/behavior/task-runtime-task-record.test.mjs",
  "tests/behavior/task-runtime-services.test.mjs",
  "tests/behavior/runtime-graph-replay.test.mjs",
  "tests/behavior/runtime-graph-scheduler.test.mjs",
  "tests/behavior/task-runtime-submission.test.mjs",
  "tests/behavior/task-runtime-cancellation.test.mjs",
  "tests/behavior/context-compiler.test.mjs",
  "tests/behavior/conversation-session-service.test.mjs",
  "tests/behavior/follow-up-resolver.test.mjs",
  "tests/behavior/followup-artifact-eval-corpus.test.mjs",
  "tests/behavior/artifact-extract-service.test.mjs",
  "tests/behavior/artifact-extract-background-lane.test.mjs",
  "tests/behavior/artifact-lineage-service.test.mjs",
  "tests/behavior/artifact-transform-service.test.mjs",
  "tests/behavior/user-memory-profile.test.mjs",
  "tests/behavior/tool-call-validator-document.test.mjs",
  "docs/scheduler/README.md",
  "docs/scheduler/cron_cheatsheet.md",
  "docs/scheduler/trigger_types.md",
  "docs/scheduler/misfire_policies.md",
  "docs/runtime/README.md",
  "docs/phase_1a_demo_script.md",
  "office_addin/README.md",
  "office_addin/shared/task_pane.html",
  "office_addin/shared/index.js",
  "office_addin/shared/office_bridge.js",
  "office_addin/shared/office_runtime.js",
  "office_addin/word/manifest.xml",
  "office_addin/excel/manifest.xml",
  "office_addin/ppt/manifest.xml",
  "src/shared/contracts/ai-provider.ts",
  "src/shared/contracts/code-cli.ts",
  "src/shared/contracts/mcp.ts",
  "src/shared/contracts/skill.ts",
  "src/shared/contracts/desktop-shell.ts",
  "src/shared/contracts/browser-context.ts",
  "src/shared/contracts/office-context.ts",
  "src/shared/contracts/uca-models.ts",
  "src/shared/conversation-model-override.mjs",
  "src/shared/provider-configuration.mjs",
  "src/shared/project-store.mjs",
  "src/service/capabilities/skills/lifecycle.mjs",
  "src/desktop/shared/manifest.mjs",
  "src/desktop/tray/bootstrap.mjs",
  "src/desktop/tray/runtime-host.mjs",
  "src/desktop/tray/electron-main.mjs",
  "src/desktop/tray/desktop-actor.mjs",
  "src/desktop/tray/dock-geometry.mjs",
  "src/desktop/renderer/README.md",
  "src/desktop/renderer/preload.cjs",
  "src/desktop/renderer/shared.css",
  "src/desktop/renderer/shared-ui.mjs",
  "src/desktop/renderer/capability-checklist.mjs",
  "src/desktop/renderer/capability-tool-view.mjs",
  "src/desktop/renderer/model-picker-view-model.mjs",
  "src/desktop/renderer/mcp-missing-config.mjs",
  "src/desktop/renderer/console-mcp-view.mjs",
  "src/desktop/renderer/console-skills-view.mjs",
  "src/desktop/renderer/conversation-cache.mjs",
  "src/desktop/renderer/console-chat-sidebar.mjs",
  "src/desktop/renderer/console-conversation-viewer.mjs",
  "src/desktop/renderer/console-task-timeline.mjs",
  "src/desktop/renderer/console-task-list.mjs",
  "src/desktop/renderer/console-files-view.mjs",
  "src/desktop/renderer/console-task-detail.mjs",
  "src/desktop/renderer/console-task-event-stream.mjs",
  "src/desktop/renderer/console-notes-model.mjs",
  "src/desktop/renderer/console-projects-view.mjs",
  "src/desktop/renderer/tool-display.mjs",
  "src/desktop/renderer/console.html",
  "src/desktop/renderer/console.js",
  "src/desktop/renderer/console-inbox-view.mjs",
  "src/desktop/renderer/dock.html",
  "src/desktop/renderer/dock.js",
  "src/desktop/renderer/overlay.html",
  "src/desktop/renderer/overlay.js",
  "src/desktop/renderer/overlay-project-model.mjs",
  "src/desktop/renderer/overlay-auto-tasks.mjs",
  "src/desktop/renderer/overlay-task-routing.mjs",
  "src/desktop/renderer/popup-card.html",
  "src/desktop/renderer/popup-card.js",
  "src/desktop/renderer/schedule-parser.js",
  "src/desktop/overlay/view-model.mjs",
  "src/desktop/console/view-model.mjs",
  "src/desktop/console/runtime-client.mjs",
  "src/desktop/console/privacy_settings/README.md",
  "src/desktop/console/privacy_settings/view-model.mjs",
  "src/desktop/console/first_run_wizard/README.md",
  "src/desktop/console/first_run_wizard/view-model.mjs",
  "src/desktop/console/audit_log_viewer/README.md",
  "src/desktop/console/audit_log_viewer/view-model.mjs",
  "src/desktop/console/schedules/README.md",
  "src/desktop/console/schedules/view-model.mjs",
  "src/desktop/console/pending-approvals/README.md",
  "src/desktop/console/pending-approvals/view-model.mjs",
  "src/desktop/console/task-detail/view-model.mjs",
  "src/desktop/console/filters/view-model.mjs",
  "src/desktop/console/tool-call-confirm/README.md",
  "src/desktop/console/tool-call-confirm/view-model.mjs",
  "src/desktop/console/template_editor/README.md",
  "src/desktop/console/template_editor/view-model.mjs",
  "src/desktop/console/dag_view/README.md",
  "src/desktop/console/dag_view/view-model.mjs",
  "src/desktop/console/budget_dashboard/README.md",
  "src/desktop/console/budget_dashboard/view-model.mjs",
  "src/desktop/console/history_search/README.md",
  "src/desktop/console/history_search/view-model.mjs",
  "scripts/gui-smoke-perf-contract.mjs",
  "scripts/verify-code-ownership-boundaries.mjs",
  "scripts/verify-ipc-contract-inventory.mjs",
  "scripts/verify-http-route-inventory.mjs",
  "scripts/verify-tool-registry-snapshot.mjs",
  "scripts/verify-artifact-surface-snapshot.mjs",
  "scripts/verify-renderer-direct-runtime-calls.mjs",
  "src/service/core/service-bootstrap.mjs",
  "src/service/core/persistent-runtime.mjs",
  "src/service/core/http-server.mjs",
  "src/service/core/http-route-guards.mjs",
  "src/service/core/http-routes/ai-status-routes.mjs",
  "src/service/core/http-routes/audio-routes.mjs",
  "src/service/core/http-routes/browser-context-routes.mjs",
  "src/service/core/http-routes/config-provider-routes.mjs",
  "src/service/core/http-routes/mcp-install-routes.mjs",
  "src/service/core/http-routes/note-project-conversation-routes.mjs",
  "src/service/core/http-routes/office-routes.mjs",
  "src/service/core/http-routes/preview-file-routes.mjs",
  "src/service/core/http-routes/runtime-admin-routes.mjs",
  "src/service/core/http-routes/scheduler-template-routes.mjs",
  "src/service/core/http-routes/task-routes.mjs",
  "src/service/core/http-routes/translation-routes.mjs",
  "src/service/core/capability-creator/index.mjs",
  "src/service/core/external-call.mjs",
  "src/service/core/read-probe-cache.mjs",
  "src/service/core/evidence/content-evidence.mjs",
  "src/service/core/evidence/content-evidence-gate.mjs",
  "src/service/core/task-runtime/composite-status.mjs",
  "src/service/core/task-runtime/conversation-lifecycle.mjs",
  "src/service/core/task-runtime/event-emitter.mjs",
  "src/service/core/task-runtime/event-log.mjs",
  "src/service/core/task-runtime/task-lifecycle.mjs",
  "src/service/core/task-runtime/task-record.mjs",
  "src/service/core/task-runtime/runtime-services.mjs",
  "src/service/core/task-runtime/task-submission.mjs",
  "src/service/core/task-runtime/task-cancellation.mjs",
  "src/service/core/context-submission.mjs",
  "src/service/core/config-store.mjs",
  "src/service/core/runtime-paths.mjs",
  "src/service/core/context/context-compiler.mjs",
  "src/service/core/artifact-extracts/artifact-extract-service.mjs",
  "src/service/core/artifact-lineage/artifact-lineage-service.mjs",
  "src/service/core/artifact-transforms/artifact-transform-service.mjs",
  "src/service/memory/user-profile.mjs",
  "src/service/core/session/conversation-session-service.mjs",
  "src/service/core/session/follow-up-resolver.mjs",
  "src/service/core/action-tool-submission.mjs",
  "src/service/core/browser-submission.mjs",
  "src/service/core/image-submission.mjs",
  "src/service/core/office-submission.mjs",
  "src/service/core/screenshot-submission.mjs",
  "src/service/core/task-runtime.mjs",
  "src/service/core/store/sqlite-schema.mjs",
  "src/service/core/store/memory-store.mjs",
  "src/service/core/store/sqlite-store.mjs",
  "src/service/core/events/event-bus.mjs",
  "src/service/core/queue/task-queue.mjs",
  "src/service/core/router/intent-router.mjs",
  "src/service/action_tools/README.md",
  "src/service/capabilities/registry/types.mjs",
  "src/service/capabilities/schemas/index.mjs",
  "src/service/action_tools/tool-helper.mjs",
  "src/service/capabilities/registry/risk_matrix.mjs",
  "src/service/capabilities/registry/policy-guard.mjs",
  "src/service/capabilities/registry/registry.mjs",
  "src/service/action_tools/tools/index.mjs",
  "src/service/capabilities/tools/file-reversibility.mjs",
  "src/service/capabilities/tools/desktop-capture-gui-tools.mjs",
  "src/service/capabilities/tools/desktop-launch-tools.mjs",
  "src/service/capabilities/tools/file-content-tools.mjs",
  "src/service/capabilities/tools/file-mutation-execution-tools.mjs",
  "src/service/capabilities/tools/document-artifact-helpers.mjs",
  "src/service/capabilities/tools/document-render-tools.mjs",
  "src/service/capabilities/tools/capability-creator-tools.mjs",
  "src/service/capabilities/tools/mermaid-assets.mjs",
  "src/service/capabilities/tools/svg-sanitize.mjs",
  "src/service/core/file-submission.mjs",
  "src/service/store/artifact-store.mjs",
  "src/service/extractors/file-ingest.mjs",
  "src/service/extractors/extract-office-openxml.ps1",
  "src/service/extractors/image_ocr.mjs",
  "src/service/extractors/pdf_ocr.mjs",
  "src/service/extractors/pdf_table.mjs",
  "src/service/extractors/pdf_text.mjs",
  "src/service/executors/fast/fast-executor.mjs",
  "src/service/executors/kimi/kimi-cli-executor.mjs",
  "src/service/executors/kimi/output-format.mjs",
  "src/service/executors/kimi/print-mode-prompt.mjs",
  "src/service/executors/kimi/task-package-builder.mjs",
  "src/service/executors/shared/local-file-read-guidance.mjs",
  "src/service/executors/shared/loop-policy.mjs",
  "src/service/executors/shared/artifact-tool-preflight.mjs",
  "src/service/executors/tool_using/README.md",
  "src/service/executors/tool_using/action-guidance.mjs",
  "src/service/executors/tool_using/agent-loop.mjs",
  "src/service/executors/tool_using/finalization.mjs",
  "src/service/executors/tool_using/final-composer.mjs",
  "src/service/executors/tool_using/launch-sequence.mjs",
  "src/service/executors/tool_using/loop-policy.mjs",
  "src/service/executors/tool_using/planner-formatting.mjs",
  "src/service/executors/tool_using/planner-mode.mjs",
  "src/service/executors/tool_using/confirmation-gate.mjs",
  "src/service/executors/tool_using/error-budget-gate.mjs",
  "src/service/executors/tool_using/phase-gate.mjs",
  "src/service/executors/tool_using/side-effect-gate.mjs",
  "src/service/executors/tool_using/scheduled-fire-gate.mjs",
  "src/service/executors/tool_using/saturation-gate.mjs",
  "src/service/executors/tool_using/conversation-messages.mjs",
  "src/service/executors/tool_using/truthfulness-guard.mjs",
  "src/service/executors/tool_using/tool-call-guards.mjs",
  "src/service/executors/tool_using/tool-arg-repair.mjs",
  "src/service/executors/tool_using/tool-surface.mjs",
  "src/service/executors/tool_using/tool-call-validator.mjs",
  "src/service/executors/agentic/finalization.mjs",
  "src/service/executors/agentic/tool-execution.mjs",
  "src/service/executors/agentic/tool-surface.mjs",
  "src/service/executors/agentic/user-message.mjs",
  "src/service/executors/agentic/tool-result-controls.mjs",
  "src/service/executors/agentic/validator-transcript.mjs",
  "src/service/executors/multi_modal/README.md",
  "src/service/executors/multi_modal/multi-modal-executor.mjs",
  "src/service/executors/registry.mjs",
  "src/service/templates/README.md",
  "src/service/templates/schema.mjs",
  "src/service/templates/parser.mjs",
  "src/service/templates/runtime.mjs",
  "src/service/templates/builtin/index.mjs",
  "src/service/templates/builtin/legal_contract_review.json",
  "src/service/templates/builtin/academic_summary.json",
  "src/service/templates/builtin/data_briefing.json",
  "src/service/templates/builtin/email_draft.json",
  "src/service/templates/builtin/competitor_scan.json",
  "src/service/dag/README.md",
  "src/service/dag/scheduler.mjs",
  "src/service/dag/visualizer.mjs",
  "src/service/cost/README.md",
  "src/service/cost/pricing.json",
  "src/service/cost/estimator.mjs",
  "src/service/cost/budget.mjs",
  "src/service/embeddings/README.md",
  "src/service/embeddings/bge_local.mjs",
  "src/service/embeddings/store.mjs",
  "src/service/events/sse.mjs",
  "src/service/failures/classifier.mjs",
  "src/service/failures/user_messages.json",
  "src/service/retry/retry-manager.mjs",
  "src/service/executors/shared/code-cli-invocation.mjs",
  "src/service/metrics/registry.mjs",
  "src/service/security/README.md",
  "src/service/security/broker.mjs",
  "src/service/security/audit-log.mjs",
  "src/service/security/kill-switch.mjs",
  "src/service/security/screen-share-monitor.mjs",
  "src/service/security/rules/defaults.json",
  "src/service/security/rules/blocklist.mjs",
  "src/service/security/rules/pii_redaction.mjs",
  "src/service/https/README.md",
  "src/service/https/self-signed-cert.mjs",
  "src/service/https/port-9413.mjs",
  "src/service/core/windows-pipe-server.mjs",
  "src/service/scheduler/README.md",
  "src/service/scheduler/store.mjs",
  "src/service/scheduler/nl_to_cron.mjs",
  "src/service/scheduler/misfire.mjs",
  "src/service/scheduler/failure_guard.mjs",
  "src/service/scheduler/pending-approvals.mjs",
  "src/service/scheduler/execute-action.mjs",
  "src/service/scheduler/dispatch.mjs",
  "src/service/scheduler/engine.mjs",
  "uca-cli/src/submit.mjs",
  "uca-cli/src/runtime-client.mjs",
  "uca-cli/src/cli.mjs",
  "src/helper/explorer_selection/selection-contract.mjs",
  "src/helper/explorer_selection/UcaExplorerSelectionHelper/UcaExplorerSelectionHelper.csproj",
  "src/helper/explorer_selection/UcaExplorerSelectionHelper/Program.cs",
  "src/helper/Screenshot/README.md",
  "src/helper/Screenshot/capture-contract.mjs",
  "scripts/local-whisper-transcribe.py",
  "scripts/verify-desktop-shell.mjs",
  "scripts/verify-desktop-renderer.mjs",
  "scripts/verify-overlay-composer.mjs",
  "scripts/verify-context-handoff-ui.mjs",
  "scripts/verify-console-ui.mjs",
  "scripts/verify-console-runtime-client.mjs",
  "scripts/verify-console-rendered-workspace.mjs",
  "scripts/verify-console-inbox-view.mjs",
  "scripts/check-manifest.mjs",
  "scripts/run-checks.mjs",
  "scripts/verify-check-runner.mjs",
  "scripts/verify-service-core.mjs",
  "scripts/verify-provider-health.mjs",
  "scripts/verify-file-kimi.mjs",
  "scripts/verify-kimi-runtime.mjs",
  "scripts/verify-browser-extension.mjs",
  "scripts/verify-browser-overlay.mjs",
  "scripts/verify-pdf-ocr.mjs",
  "scripts/verify-office-base.mjs",
  "scripts/verify-status-metrics.mjs",
  "scripts/verify-action-tools.mjs",
  "scripts/verify-security-broker.mjs",
  "scripts/verify-scheduler.mjs",
  "scripts/verify-platform-foundation.mjs",
  "scripts/verify-template-dag-persistence.mjs",
  "scripts/verify-runtime-wiring.mjs",
  "scripts/verify-runtime-upgrade-guardrails.mjs",
  "scripts/verify-runtime-performance-baseline.mjs",
  "scripts/verify-main-process-blocking.mjs",
  "scripts/verify-renderer-stream-batching.mjs",
  "scripts/verify-context-compiler-boundary.mjs",
  "scripts/verify-context-compiler-v1.mjs",
  "scripts/verify-conversation-session-foundation.mjs",
  "scripts/verify-follow-up-resolver-foundation.mjs",
  "scripts/verify-artifact-extract-foundation.mjs",
  "scripts/verify-artifact-extract-background-lane.mjs",
  "scripts/verify-artifact-lineage-contracts.mjs",
  "scripts/verify-artifact-transform-flows.mjs",
  "scripts/verify-memory-governance.mjs",
  "scripts/verify-session-compaction.mjs",
  "scripts/verify-context-debug-panel-lazy-load.mjs",
  "scripts/verify-runtime-graph-nodes.mjs",
  "scripts/verify-runtime-graph-replay.mjs",
  "scripts/verify-runtime-graph-scheduling-budget.mjs",
  "scripts/verify-followup-artifact-eval-corpus.mjs",
  "scripts/verify-ai-integrations.mjs",
  "scripts/verify-behavior-tests.mjs",
  "scripts/verify-doc-references.mjs",
  "scripts/verify-github-readiness.mjs",
  "scripts/verify-dependency-hygiene.mjs",
  "scripts/verify-github-pages-readiness.mjs",
  "scripts/verify-external-call-boundary.mjs",
  "scripts/verify-submission-policy-boundary.mjs",
  "scripts/verify-local-http-surface.mjs",
  "scripts/verify-native-integrations.mjs",
  "scripts/create-ooxml-fixture.ps1",
  "scripts/render-document.ps1",
  "scripts/build-trial-package.mjs",
  "scripts/verify-release-readiness.mjs",
  "scripts/install-explorer-entry.ps1",
  "scripts/install-native-host.ps1",
  "scripts/check-trial-prereqs.ps1",
  "scripts/generate-trial-readiness-report.mjs",
  "scripts/generate-third-party-licenses.mjs",
  "scripts/verify-trial-launch.ps1",
  "scripts/verify-trial-launch.mjs",
  "scripts/setup-trial.ps1",
  "scripts/submit-file-task.ps1",
  "src/service/capabilities/providers/README.md",
  "src/service/capabilities/providers/registry.mjs",
  "src/service/capabilities/providers/builtin.mjs",
  "src/service/capabilities/providers/configured.mjs",
  "src/service/capabilities/providers/model-discovery.mjs",
  "src/service/capabilities/providers/runtime.mjs",
  "src/service/ai/integrations/runtime.mjs",
  "src/service/capabilities/code_cli/README.md",
  "src/service/capabilities/code_cli/registry.mjs",
  "src/service/capabilities/code_cli/configured.mjs",
  "src/service/capabilities/code_cli/builtin.mjs",
  "src/service/capabilities/code_cli/kimi/README.md",
  "src/service/capabilities/code_cli/kimi/runtime.mjs",
  "src/service/capabilities/mcp/README.md",
  "src/service/capabilities/mcp/registry.mjs",
  "src/service/capabilities/mcp/configured.mjs",
  "src/service/capabilities/mcp/builtin.mjs",
  "src/service/capabilities/mcp/drafts.mjs",
  "src/service/capabilities/mcp/env-resolver.mjs",
  "src/service/capabilities/mcp/discovery-catalog.mjs",
  "src/service/capabilities/mcp/install-detection.mjs",
  "src/service/capabilities/mcp/install-execution.mjs",
  "src/service/capabilities/mcp/install-sandbox.mjs",
  "src/service/capabilities/skills/README.md",
  "src/service/capabilities/skills/registry.mjs",
  "src/service/capabilities/skills/builtin.mjs",
  "docs/runtime/kimi_cli_setup.md",
  "docs/runtime/file_entry_setup.md",
  "docs/runtime/install_extension.md",
  "docs/runtime/local_runtime.md",
  "docs/runtime/ai_integrations.md",
  "docs/runtime/native_integrations.md",
  "docs/runtime/native_messaging_protocol.md",
  "docs/operations/README.md",
  "docs/operations/failure_taxonomy.md",
  "docs/operations/retry_strategies.md",
  "external/paddle_ocr_runtime/README.md",
  "uca-native-host/README.md",
  "uca-native-host/index.mjs",
  "uca-native-host/protocol.mjs",
  "uca-native-host/registry-manifest.mjs",
  "uca-native-host/UcaNativeHost/UcaNativeHost.csproj",
  "uca-native-host/UcaNativeHost/Program.cs",
  "browser_ext/manifest.json",
  "browser_ext/background/service-worker.js",
  "browser_ext/content_script/rules.js",
  "browser_ext/content_script/placement.js",
  "browser_ext/content_script/stability-watcher.js",
  "browser_ext/content_script/selection-cache.js",
  "browser_ext/popup/index.html",
  "browser_ext/popup/index.js",
  "browser_ext/popup/styles.css",
  "browser_ext/shadow_ui/floating-chip.js",
  "tests/fixtures/mock-kimi-cli.mjs",
  "tests/fixtures/mock-failing-kimi-cli.mjs",
  "tests/fixtures/mock-slow-kimi-cli.mjs",
  "tools/release/README.md",
  "tools/release/release-config.json"
];

const missing = requiredPaths.filter((path) => !existsSync(path));

if (missing.length > 0) {
  console.error("Missing required paths:");
  for (const path of missing) {
    console.error(`- ${path}`);
  }
  process.exit(1);
}

const agentLoopSource = readFileSync("src/service/executors/tool_using/agent-loop.mjs", "utf8");
if (/text\.includes\(["']通知["']\)|text\.includes\(["']notify["']\)/.test(agentLoopSource)) {
  throw new Error("defaultPlanner must not route tools from per-word text.includes checks.");
}

const topicHintSource = readFileSync("src/service/core/intent/signals/topic-hint.mjs", "utf8");
if (/const\s+PATTERN\s*=/.test(topicHintSource) || /天气\|气温|weather\|forecast|stock\\s\*price|AI\\s\*新闻/i.test(topicHintSource)) {
  throw new Error("topic_hint must remain a compatibility placeholder; topic regex belongs to SemanticRouter.");
}

const submitFileTaskScript = readFileSync("scripts/submit-file-task.ps1", "utf8");
if (!/background\s*=\s*\$true/i.test(submitFileTaskScript)) {
  throw new Error("submit-file-task.ps1 must request background task creation before polling status.");
}

console.log("Repository structure verification passed.");
