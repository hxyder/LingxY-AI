...
function Get-OfficeAddinSetupStatus {
  ...
  if ($share) {
    $sharePathMatches = [string]$share.Path -eq $CatalogPath
  }
  ...
  return [ordered]@{
    ok = ((Test-Path -LiteralPath $CatalogPath) -and $share -and $sharePathMatches -and $shareReadable -and $registryTrusted)
    ...
    sharePathMatches = $sharePathMatches
