...
// Fast path wins first.
{
  ...
  assert.equal(r.lane, "fast_path");
  assert.equal(r.tool, "open_url");
}

// Schedule intercept wins over single_turn when handleAsPlan handled it.
{
  const runtime = makeRuntime();
  const fakeTask = { task_id: "t_fake", sub_status: "scheduled" };
  const r = await triage({
    runtime,
    userCommand: "5 分钟后发美股汇总",
    fastPath: () => null,
    handleAsPlan: async () => ({ handled: true, task: fakeTask, schedule: { schedule_id: "s_fake" }, message: "已安排" })
  });
  assert.equal(r.lane, "schedule");
  assert.equal(r.task.task_id, "t_fake");
}

// Clarify lane surfaces when plan layer asks a question.
{
  const runtime = makeRuntime();
  const fakeTask = { task_id: "t_clarify", sub_status: "clarify" };
  const r = await triage({
    runtime,
    userCommand: "5 分钟后帮我做点事",
    fastPath: () => null,
    handleAsPlan: async () => ({ handled: true, task: fakeTask, message: "请问你想做什么？" })
  });
  assert.equal(r.lane, "clarify");
  assert.ok(r.message?.includes("做什么"));
}

// Rewritten command substitutes forward into single_turn.
{
  const runtime = makeRuntime();
  const r = await triage({
    runtime,
    userCommand: "原命令",
    fastPath: () => null,
    handleAsPlan: async () => ({ handled: false, rewrittenCommand: "更精细的命令" })
  });
  assert.equal(r.lane, "single_turn");
  assert.equal(r.userCommand, "更精细的命令");
}
