...
$fgHwnd = [UcaCapture]::GetForegroundWindow()
...
function Get-ExplorerSelection {
    ...
    try {
        ...
        foreach ($window in @($shell.Windows())) {
            try {
                $windowHwndValue = [int64]$window.HWND
                if ($windowHwndValue -ne $foregroundHwndValue) {
                    continue
                }
...
if ($procName -eq "explorer") {
    $explorerSelection = Get-ExplorerSelection -ForegroundHwnd $fgHwnd
}
...
if ($SimulateCopy -and $files.Count -gt 0) {
    $baselineFiles = if ($preCopySnapshot) { Join-ClipboardFiles $preCopySnapshot.files } else { "" }
    $currentFiles = Join-ClipboardFiles $files
    if ($baselineFiles -and $currentFiles -eq $baselineFiles) {
        $files = @()
    }
}
