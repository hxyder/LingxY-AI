...
} from "../src/service/capabilities/tools/file-content-tools.mjs";
...
// Post-move state: this family is owned by capabilities/tools and index.mjs
// only aggregates the tools.

const aggregatorPath = "src/service/action_tools/tools/index.mjs";
const ownerPath = "src/service/capabilities/tools/file-content-tools.mjs";
const oldOwnerPath = "src/service/action_tools/tools/file-content-tools.mjs";
...
assert(existsSync(path.join(root, ownerPath)), `file content/artifact owner missing: ${ownerPath}`);
assert(existsSync(path.join(root, boundaryPath)), `file content/artifact boundary doc missing: ${boundaryPath}`);
assert(!existsSync(path.join(root, oldOwnerPath)), `old file content/artifact owner must not exist: ${oldOwnerPath}`);
...
for (const requiredText of [
  ...
  assert(ownerSrc.includes(requiredText), `file content/artifact owner missing ${requiredText}`);
}
...
for (const removedText of [
  ...
  assert(!indexSrc.includes(removedText), `index.mjs must not retain old file content/artifact owner text: ${removedText}`);
}
...
for (const [id, risk, requiresConfirmation, capabilities] of expected) {
  ...
  const expectedTool = {
    read_file_text: READ_FILE_TEXT_TOOL,
    read_folder_text: READ_FOLDER_TEXT_TOOL,
    search_file_content: SEARCH_FILE_CONTENT_TOOL,
    index_file_content: INDEX_FILE_CONTENT_TOOL,
    register_artifact: REGISTER_ARTIFACT_TOOL,
    resolve_output_path: RESOLVE_OUTPUT_PATH_TOOL
  }[id];
  assert.equal(tool, expectedTool, `${id} must be aggregated from file-content-tools.mjs`);
...
const readFileStart = ownerSrc.indexOf("export const READ_FILE_TEXT_TOOL");
const readFolderStart = ownerSrc.indexOf("export const READ_FOLDER_TEXT_TOOL");
const searchStart = ownerSrc.indexOf("export const SEARCH_FILE_CONTENT_TOOL");
const indexStart = ownerSrc.indexOf("export const INDEX_FILE_CONTENT_TOOL");
const registerStart = ownerSrc.indexOf("export const REGISTER_ARTIFACT_TOOL");
const resolveStart = ownerSrc.indexOf("export const RESOLVE_OUTPUT_PATH_TOOL");
...
const indexToolSrc = ownerSrc.slice(indexStart, registerStart);
...
const resolveToolSrc = ownerSrc.slice(resolveStart);
...
console.log("[file-content-tools] contract verified");
