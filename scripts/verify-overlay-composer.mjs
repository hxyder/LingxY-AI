...
assert.equal(overlayJs.includes("overlayShellClient.notify"), true);
...
assert.equal(overlayJs.includes("overlayViewGeneration"), true);
assert.equal(overlayJs.includes("taskFrameVisibilityForCurrentOverlay"), true);
assert.match(overlayJs, /function startNewConversation[\s\S]{0,320}advanceOverlayViewGeneration\(\)/);
assert.match(overlayJs, /function appendTurnForTask[\s\S]{0,360}shouldRenderTaskFrameInCurrentOverlay\(taskId\)/);
assert.match(overlayJs, /function replayTaskTimelineEvents[\s\S]{0,420}shouldRenderTaskFrameInCurrentOverlay/);
assert.match(overlayJs, /async function refreshActiveTask\(\)[\s\S]{0,720}isActiveTaskRefreshCurrent\(refreshTaskId, refreshViewGeneration\)/);
...
assert.equal(overlayJs.includes("selectPartialSuccessTaskMessage"), true);
assert.equal(overlayJs.includes("notifiedInlineResultTaskId !== task.task_id"), true);
...
const overlayTaskRouting = await read("src/desktop/renderer/overlay-task-routing.mjs");
assert.equal(overlayTaskRouting.includes("resolveOverlayTaskEventVisibility"), true);
assert.equal(overlayTaskRouting.includes("no_visible_conversation"), true);
assert.equal(overlayTaskRouting.includes("owner_mismatch"), true);
assert.equal(overlayTaskRouting.includes("active_task_without_owner"), true);
