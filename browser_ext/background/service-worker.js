...
export async function dispatchExplainPage({
  ...
  let payload = null;
  try {
    payload = await capturePageSourceFromTab(tab.id, chromeApi);
  } catch (error) {
...
async function readInstalledPageSourceCapture(tabId, chromeApi = chrome) {
...
async function capturePageSourceFromTab(tabId, chromeApi = chrome) {
  let payload = await readInstalledPageSourceCapture(tabId, chromeApi);
  if (payload) return payload;

  await chromeApi.scripting.executeScript({
    target: { tabId },
    world: "MAIN",
    files: ["content_script/page-source-capture.js"]
  });
  payload = await readInstalledPageSourceCapture(tabId, chromeApi);
  return payload ?? null;
}
