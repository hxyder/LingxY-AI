...
// ── Config persistence (user overrides + packaged public OAuth defaults) ─────

export function loadConnectorConfig(runtime, type) {
  return resolveConnectorOAuthConfig(runtime, type);
}
...
function appendClientSecretIfPresent(params, clientSecret) {
  const value = typeof clientSecret === "string" ? clientSecret.trim() : "";
  if (value) params.set("client_secret", value);
  return params;
}
...
async function refreshGoogleTokens(tokens, clientId, clientSecret) {
  if (!tokens?.refresh_token) return null;
  const params = appendClientSecretIfPresent(new URLSearchParams({
    client_id: clientId,
    grant_type: "refresh_token",
    refresh_token: tokens.refresh_token
  }), clientSecret);
