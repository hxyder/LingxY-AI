...
function requestBodyHasExplicitInputContext(body = {}, {
  filePaths = [],
  imagePaths = [],
  selectionMetadata = {}
} = {}) {
  if (filePaths.length > 0 || imagePaths.length > 0) return true;
  if (body.officeCapture?.officeApp) return true;
  if (body.capture?.sourceType) {
    return contextPacketHasExplicitInputContext({
      source_type: body.capture.sourceType,
      source_app: body.capture.browser ?? body.capture.processName ?? body.sourceApp,
      capture_mode: body.captureMode ?? "browser_capture",
      text: body.capture.text ?? "",
      url: body.capture.url,
      selection_metadata: {
        ...(body.capture.selectionMetadata ?? {}),
        ...selectionMetadata
      },
      file_paths: body.capture.filePaths,
      image_paths: body.capture.imagePaths
    });
  }
  if (body.contextPacket && typeof body.contextPacket === "object") {
    return contextPacketHasExplicitInputContext(body.contextPacket);
  }
  return contextPacketHasExplicitInputContext({
...
export async function submitTaskFromBody(runtime, body) {
  ...
  const requestHasExplicitInputContext = requestBodyHasExplicitInputContext(body, {
  ...
  const effectiveRequestParentTaskId = requestParentTaskId && looksLikeFollowUpSignal(userCommand)
    && !requestHasExplicitInputContext
    ? requestParentTaskId
    : null;
