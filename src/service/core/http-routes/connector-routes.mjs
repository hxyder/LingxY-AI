...
import { serializeConnectorConfigForClient } from "../../capabilities/connectors/oauth-defaults.mjs";
import {
...
export async function tryHandleConnectorRoute({ request, response, url, method, runtime }) {
  ...
  if (method === "GET" && /^\/connectors\/accounts\/(microsoft|google)\/config$/.test(url.pathname)) {
    ...
    sendJson(response, 200, serializeConnectorConfigForClient(runtime, type));
    return true;
  }
