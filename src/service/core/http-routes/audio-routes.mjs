...
import {
  ...
  createLocalSpeechProcessEnv,
  getLocalSpeechPythonCommand,
  getLocalSpeechScriptPath,
  resolveLocalSpeechRuntime
} from "../../audio/local-speech-runtime.mjs";
import {
  providerPublicDescriptor,
  resolveAudioTranscriptionProvider,
  resolveAudioTranscriptionStatus,
  transcribeWithOpenAiCompatibleProvider
} from "../../audio/speech-provider-contract.mjs";
...
function getLocalTranscriptionScriptPath() {
  return getLocalSpeechScriptPath("whisper");
}

function getLocalKeywordSpottingScriptPath() {
  return getLocalSpeechScriptPath("sherpaKws");
}

function getPythonCommand() {
  return getLocalSpeechPythonCommand();
}
...
async function getLocalKeywordSpottingStatus() {
  try {
    const { stdout } = await execFileAsync(getPythonCommand(), [
      ...
      maxBuffer: 1024 * 1024,
      env: createLocalSpeechProcessEnv(),
      encoding: "utf8"
    });
...
async function transcribeAudioLocally(audioBuffer, { mimeType = "audio/webm", lang = "auto", noVad = false } = {}) {
  ...
  const localSpeech = resolveLocalSpeechRuntime();
  const fallbackModel = localSpeech.localWhisper.model || DEFAULT_LOCAL_WHISPER_MODEL;
...
async function transcribeAudioLocallyStream(audioBuffer, {
  ...
  try {
    ...
    const child = spawn(getPythonCommand(), [
      ...
      "--model", resolveLocalSpeechRuntime().localWhisper.model || DEFAULT_LOCAL_WHISPER_MODEL,
  ...
  if (method === "POST" && url.pathname === "/note/transcribe") {
    ...
    if (url.searchParams.get("stream") === "1") {
      ...
      if (provider) {
        ...
        const cloudResult = await transcribeWithOpenAiCompatibleProvider({
          provider,
          audioBuffer,
          mimeType,
          lang,
        ...
        if (cloudResult.ok) {
          const text = normalizeTranscriptionTextForLocale(cloudResult.result?.text ?? "", { outputLocale, lang });
    ...
    const cloudResult = await transcribeWithOpenAiCompatibleProvider({
      ...
      mimeType,
      lang,
      prompt: transcriptionPromptForLanguage(lang, outputLocale)
    });
    if (cloudResult.ok) {
      const transcript = normalizeTranscriptionTextForLocale(cloudResult.result?.text ?? "", { outputLocale, lang });
      sendJson(response, 200, {
        ok: true,
        transcript,
        provider: providerPublicDescriptor(provider)
    ...
    if (localResult.ok) {
      ...
      sendJson(response, 200, {
        ...
        fallbackFrom: {
          provider: providerPublicDescriptor(provider),
          reason: cloudResult.reason ?? "transcription_failed",
