...
const RECENT_BROWSER_CONTEXT_TEXT_LIMIT = 24000;
...
function browserContextTextLength(context = {}) {
  const metadataLength = Number(context?.metadata?.textLength ?? context?.metadata?.lengthChars);
  if (Number.isFinite(metadataLength) && metadataLength > 0) return metadataLength;
  return `${context?.text ?? ""}`.replace(/\s+/g, " ").trim().length;
}

function contextAgeMs(context = {}) {
  const receivedAt = Date.parse(context.receivedAt ?? context.metadata?.capturedAt ?? "") || 0;
  return receivedAt ? Math.max(0, Date.now() - receivedAt) : Number.POSITIVE_INFINITY;
}
...
function normalizeBrowserContextPayload(body = {}) {
  ...
  return {
    ...
    text: truncateString(raw.text ?? "", RECENT_BROWSER_CONTEXT_TEXT_LIMIT),
    metadata: {
      capturedAt,
      ...
      textLength: Number.isFinite(Number(metadata.textLength ?? metadata.lengthChars))
        ? Number(metadata.textLength ?? metadata.lengthChars)
        : truncateString(raw.text ?? "", RECENT_BROWSER_CONTEXT_TEXT_LIMIT).length,
      textTruncated: Boolean(metadata.textTruncated)
        || String(raw.text ?? "").replace(/\s+/g, " ").trim().length > RECENT_BROWSER_CONTEXT_TEXT_LIMIT,
      contentScope: truncateString(metadata.contentScope ?? "document_text_snapshot", 80),
      youtube: youtube ? {
        platform: "youtube",
...
function scoreBrowserContext(context, {
  url = "",
  title = "",
  ...
  if (url && contextUrl) {
    ...
    } else if (safeUrlHost(contextUrl) && safeUrlHost(contextUrl) === safeUrlHost(url)) {
      score += 18;
      matchReason = "same_host";
    }
  }
  ...
  if (normalizedTitle && normalizedContextTitle) {
    if (normalizedTitle.includes(normalizedContextTitle) || normalizedContextTitle.includes(normalizedTitle)) {
      score += 55;
      if (matchReason === "none" || matchReason === "same_host") matchReason = "title";
    } else {
      ...
      const overlapScore = Math.min(40, overlap * 6);
      score += overlapScore;
      if (overlapScore > 0 && (matchReason === "none" || matchReason === "same_host")) {
        matchReason = "title_tokens";
      }
    }
  }

  const ageMs = contextAgeMs(context);
  if (Number.isFinite(ageMs)) score += Math.max(0, 20 - Math.floor(ageMs / 60_000));

  const hasExplicitMatch = matchReason !== "none";
  const hasEnoughText = browserContextTextLength(context) >= minTextLength;
  const withinRecentWindow = Number.isFinite(ageMs) && (!maxAgeMs || ageMs <= maxAgeMs);
  if (!hasExplicitMatch && allowRecentFallback && withinRecentWindow && hasEnoughText) {
    score += 10;
    matchReason = "recent_text_snapshot";
  }
...
export function listRecentBrowserContexts(recentBrowserContexts, query = {}) {
  ...
  const allowRecentFallback = Boolean(query.allowRecentFallback);
  const maxAgeMs = Math.max(0, Number(query.maxAgeMs ?? 0) || 0);
  const minTextLength = Math.max(0, Number(query.minTextLength ?? 0) || 0);
  const requireText = Boolean(query.requireText);
  return recentBrowserContexts
    .map((context) => {
      const scored = scoreBrowserContext(context, {
        ...query,
        allowRecentFallback,
        maxAgeMs,
        minTextLength
      });
      return { context, score: scored.score, matchReason: scored.matchReason };
    })
    .filter((item) => {
    ...
    .filter((item) => {
      ...
      return browserContextTextLength(item.context) >= minTextLength;
    })
    .filter((item) => {
      if (!maxAgeMs) return true;
      if (item.matchReason === "recent_text_snapshot") return contextAgeMs(item.context) <= maxAgeMs;
      if (allowRecentFallback && !query.url && !query.title) return contextAgeMs(item.context) <= maxAgeMs;
      return true;
    })
    .filter((item) => !query.url && !query.title ? true : item.matchReason !== "none")
    .sort((a, b) => b.score - a.score)
    .slice(0, limit)
    .map((item) => ({ ...item.context, score: item.score, matchReason: item.matchReason }));
}
...
export async function tryHandleBrowserContextRoute({ request, response, method, url, runtime, recentBrowserContexts }) {
  ...
  if (method === "GET" && url.pathname === "/browser/context/recent") {
    const contexts = listRecentBrowserContexts(recentBrowserContexts, {
      ...
      allowRecentFallback: url.searchParams.get("allow_recent_fallback") === "1",
      requireText: url.searchParams.get("require_text") === "1",
      minTextLength: url.searchParams.get("min_text_length") ?? "0",
      maxAgeMs: url.searchParams.get("max_age_ms") ?? "0",
