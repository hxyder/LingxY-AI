...
const STANDALONE_FILE_CREATION_REQUEST = /^(?:请)?\s*(?:生成|创建|制作|保存|导出|写入|写成|做一个|create|generate|save|export|write|make).{0,80}(?:文件|文档|\.mjs|\.js|\.py|\.ps1|\.md|\.json|\.csv|\.txt|html|markdown|json|csv|script|file|document)/iu;
const EXPLICIT_PRIOR_ARTIFACT_CONTEXT = /(上个|上一个|上一轮|刚才|之前|前面|已有|现有|原文件|原文档|latest|last|previous|existing).{0,40}(文件|文档|artifact|file|document)/iu;
...
const EXPLICIT_CONTEXT_SOURCE_TYPES = new Set([
  "file",
  "file_group",
  "image",
  "image_group",
...
function hasUsableContentEvidence(selectionMetadata = {}) {
  const evidences = Array.isArray(selectionMetadata?.content_evidence)
    ? selectionMetadata.content_evidence
    : [];
  return evidences.some((entry) =>
    entry?.status === "available"
    && (
      entry.content_extracted === true
      || entry.pixels_available === true
      || entry.source_kind === "attached_image"
      || entry.source_kind === "browser_page_text"
      || entry.source_kind === "file_text"
    )
  );
}

export function contextPacketHasExplicitInputContext(contextPacket = {}) {
  if (!contextPacket || typeof contextPacket !== "object") return false;
  const selectionMetadata = contextPacket.selection_metadata ?? contextPacket.selectionMetadata ?? {};
  if (hasNonEmptyArray(contextPacket.file_paths) || hasNonEmptyArray(contextPacket.image_paths)) return true;
  if (hasUsableContentEvidence(selectionMetadata)) return true;
  if (selectionMetadata.active_window_context === true
      || selectionMetadata.current_window_context === true
      || selectionMetadata.browser_page_content === true
      || selectionMetadata.file_ingest_status === "finished"
      || selectionMetadata.file_ingest_status === "pending") {
    return true;
  }
  const sourceType = String(contextPacket.source_type ?? contextPacket.sourceType ?? "").trim();
  const text = String(contextPacket.text ?? "").trim();
  if (text && EXPLICIT_CONTEXT_SOURCE_TYPES.has(sourceType)) return true;
  if (contextPacket.url && ["webpage", "link"].includes(sourceType)) return true;
  return false;
}
...
export function looksLikeFollowUpSignal(userCommand = "") {
  ...
  if (STANDALONE_FILE_CREATION_REQUEST.test(text)
      && !EXPLICIT_PRIOR_ARTIFACT_CONTEXT.test(text)) return false;
...
export function resolveFollowUp({
  ...
  if (contextPacketHasExplicitInputContext(contextPacket)) {
    return makeResolution({
      isFollowUpSignal,
      reason: "explicit current input context supersedes prior session anchors"
    });
  }
