...
function detectRecoveryIssue(errorLike = {}, text = "") {
  ...
  if (/planner_provider_failed|model provider connection failed|provider connection failed|模型服务连接失败/.test(haystack)) {
...
function issueRecovery(issue) {
  if (issue === "provider_connection") {
    return {
      hint: "当前模型 provider 连接失败：请检查网络、Base URL、模型名称/API Key，或切换到另一个可用 provider 后重试。",
      actions: ["检查当前 provider 设置", "切换可用 provider", "稍后重试"],
      retryable: true
    };
  }
  return null;
...
export function classifyFailure(errorLike) {
  ...
  if (!explicitCategory) {
    ...
    } else if (code.includes("planner_provider_failed") || message.includes("模型服务连接失败") || message.includes("model provider connection failed")) {
  ...
  if (!explicitCategory && issue === "disk") {
    ...
    if (issue === "auth" || issue === "permission") {
      category = "permission_denied";
    } else if (provider && ["rate_limit", "context_limit", "model_unavailable", "provider_connection"].includes(issue)) {
      category = "model_call_error";
    } else if (issue === "provider_connection") {
      category = "model_call_error";
  ...
  const issueHint = issueRecovery(issue);
  const recoveryHint = [providerHint?.hint, toolHint?.hint, issueHint?.hint].filter(Boolean).join(" ");
  if (recoveryHint && !userMessage.includes(recoveryHint)) {
    userMessage = `${userMessage}\n${recoveryHint}`;
  }
  const retryable = providerHint?.retryable ?? toolHint?.retryable ?? issueHint?.retryable
  ...
  return {
    ...
    retryable,
    userMessage,
    userActions: uniqActions(providerHint?.actions, toolHint?.actions, issueHint?.actions, USER_ACTIONS[category]),
    internalExcerpt,
