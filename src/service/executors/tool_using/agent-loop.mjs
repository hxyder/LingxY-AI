...
async function llmPlanner({ task, transcript, tools, iteration, runtime, signal }) {
  ...
  const responseLanguageInstruction = responseLanguage && responseLanguage !== "auto"
    ? `\n\nRESPONSE LANGUAGE: The user's UI language is ${responseLanguage}. Final answers and clarifying questions must use ${responseLanguage}, even when captured context or quick-action templates contain another language.`
    : "";
...
- **No internal JSON in user replies.** Never output raw control objects or event payloads such as \`{"iteration":...,"next_action":...,"violation_kinds":...}\`. Final replies must be user-facing prose, lists, tables, or markdown in the requested response language.
...
${needsCurrentDataInstruction}${researchPrinciplesBlock}${researchBudgetBlock}${synthesisBlock}${forceToolInstruction}${scheduledFireInstruction}${responseLanguageInstruction}${mcpCapabilitiesNote}${skillCapabilitiesNote}
Use call_tool when a tool is needed. Call at most ONE tool per turn. If no tool is needed, or you need a clarification, reply with plain text only in the requested response language.`;
