...
import { classifyToolFailure } from "./failure-classifier.mjs";
...
const PLANNER_PROVIDER_TRANSPORT_MAX_ATTEMPTS = 2;
const PLANNER_PROVIDER_TRANSPORT_RETRY_DELAY_MS = 350;
...
function plannerProviderFailureKind(error) {
  return classifyToolFailure({
    error: error?.message ?? String(error ?? ""),
    observation: error?.cause?.message ?? ""
  }).kind;
}

function isRetryablePlannerProviderError(error) {
  return plannerProviderFailureKind(error) === "network_unreachable";
}

function plannerProviderFailureMessage({ error, task } = {}) {
  const cjk = commandLooksCjk(task?.user_command);
  const kind = plannerProviderFailureKind(error);
  if (kind === "network_unreachable") {
    return cjk
      ? "模型服务连接失败。我已经重试过一次；请检查当前 provider 网络连接，或在设置里切换到另一个可用 provider 后重试。"
      : "The model provider connection failed after one retry. Check the active provider/network, or switch to another available provider and retry.";
  }
  if (kind === "rate_limited") {
    return cjk
      ? "模型服务触发了频率或配额限制。请稍后重试，或切换到另一个可用 provider。"
      : "The model provider hit a rate or quota limit. Retry later, or switch to another available provider.";
  }
  if (kind === "auth_missing" || kind === "provider_missing") {
    return cjk
      ? "模型服务认证或配置不可用。请检查当前 provider 的 API Key、Base URL 和模型名称。"
      : "The model provider auth/config is unavailable. Check the active provider API key, base URL, and model name.";
  }
  const raw = `${error?.message ?? ""}`.trim();
  return cjk
    ? `模型服务调用失败${raw ? `：${raw}` : ""}`
    : `The model provider call failed${raw ? `: ${raw}` : ""}`;
}

async function waitBeforePlannerProviderRetry(attempt) {
  const delayMs = PLANNER_PROVIDER_TRANSPORT_RETRY_DELAY_MS * Math.max(1, Number(attempt) || 1);
  await new Promise((resolve) => setTimeout(resolve, delayMs));
}

async function generatePlannerTextWithProviderRetry({
  adapter,
  options,
  runtime,
  task,
  ...
  let lastError = null;
  for (let attempt = 1; attempt <= PLANNER_PROVIDER_TRANSPORT_MAX_ATTEMPTS; attempt += 1) {
    try {
      return await generateTextWithContinuations(options);
    } catch (error) {
      if (error?.code === "ABORT_ERR") throw error;
      lastError = error;
      const retrying = attempt < PLANNER_PROVIDER_TRANSPORT_MAX_ATTEMPTS
        && isRetryablePlannerProviderError(error);
      runtime?.emitTaskEvent?.("planner_provider_transport_error", {
        iteration,
        attempt,
        max_attempts: PLANNER_PROVIDER_TRANSPORT_MAX_ATTEMPTS,
        retrying,
        failure_kind: plannerProviderFailureKind(error),
        provider_id: adapter?.descriptor?.provider_id ?? null,
        provider_kind: adapter?.descriptor?.provider_kind ?? adapter?.kind ?? null,
        provider_name: adapter?.descriptor?.provider_name ?? null,
        model: adapter?.model ?? null,
        message: `${error?.message ?? ""}`.slice(0, 240)
      });
      if (!retrying) throw error;
      await waitBeforePlannerProviderRetry(attempt);
    }
  }
  throw lastError ?? new Error("Planner provider failed without an error.");
}
...
Use call_tool when a tool is needed. Call at most ONE tool per turn. If no tool is needed, or you need a clarification, reply with plain text only in the user's language.`;
  ...
  try {
    ...
    try {
      ...
      const generation = await generatePlannerTextWithProviderRetry({
        adapter,
    ...
    if (error?.code === "ABORT_ERR") throw error;
    const message = plannerProviderFailureMessage({ error, task });
    runtime?.emitTaskEvent?.("planner_provider_failed", {
      iteration,
      failure_kind: plannerProviderFailureKind(error),
      message
    });
    throw Object.assign(new Error(message), {
      ...
      provider_id: adapter?.descriptor?.provider_id ?? null,
      provider_kind: adapter?.descriptor?.provider_kind ?? adapter?.kind ?? null,
      provider: adapter?.descriptor?.provider_name ?? null,
      model: adapter?.model ?? null,
      cause: error
    });
  }
  ...
  return {
    ...
    async *execute(task, { signal } = {}) {
      ...
      try {
        ...
        const text = `执行器出错：${error.message}`;
        yield {
          event_type: "failed",
          payload: {
            text,
            ...
            provider_id: error?.provider_id ?? error?.cause?.provider_id ?? null,
            provider_kind: error?.provider_kind ?? error?.cause?.provider_kind ?? null,
