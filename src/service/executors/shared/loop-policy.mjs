export function inferSearchRecencyFromText(value = "", options = {}) {
  const { recentBucket = "month", fallback = null } = options;
  ...
  if (/(今天|今日|时政|要闻|最新|最近|新闻|消息|近况|latest|recent|current|news)/i.test(text)) {
    return recentBucket;
  }
  return fallback;
}
