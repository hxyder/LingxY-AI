...
export function buildAgenticSystemPrompt({
  ...
  return [
    ...
    "3. When the user asks for a rendered document artifact (pptx / docx / xlsx / pdf / html report), call `generate_document` with the appropriate `kind`. If they ask for an explicit text/code filename such as `.md`, `.txt`, `.csv`, `.json`, or a raw `.html` file, call `write_file` with that filename and full content. If they ask to download/save an image, PDF, or other web file artifact, find a candidate when needed and call `download_file` on a direct http(s) file URL; `fetch_url_content` is for readable page text, not binary artifacts. If they ask to revise an already-generated file, locate the existing artifact path and call `edit_file` with the SAME absolute path so the file is updated in place instead of creating a new sibling. Outline shapes by kind: pptx → `{ title, subtitle?, slides: [{ heading, bullets: [string] }] }`; docx/pdf/html → `{ title, sections: [{ heading, body, bullets?, table?, diagram?, svg? }] }`; xlsx → `{ headers: [string], rows: [[cell, ...]] }` or `{ sheets: [{ name, headers, rows }] }`. XLSX must contain real tabular cells; never dump final prose, markdown tables as text, download instructions, or sandbox links into a generic `Content` column.",
