...
contextBridge.exposeInMainWorld("ucaShell", {
  ...
  captureWindowScreenshot(options = {}) {
    return ipcRenderer.invoke("uca:capture-window-screenshot", options ?? {});
  },
  // ── Shell settings ──
  getSettings() {
  ...
  getOfficeAddinsSetupStatus() {
    return ipcRenderer.invoke("uca:office-addins-setup-status");
  },
  setupOfficeAddins(payload) {
    return ipcRenderer.invoke("uca:office-addins-setup", payload ?? {});
  },
  getBrowserExtensionSetupStatus() {
    return ipcRenderer.invoke("uca:browser-extension-setup-status");
  },
  setupBrowserExtension(payload) {
    return ipcRenderer.invoke("uca:browser-extension-setup", payload ?? {});
  },
