...
const browserExtensionSetupState = document.querySelector("#browserExtensionSetupState");
const checkBrowserExtensionButton = document.querySelector("#checkBrowserExtensionButton");
const setupBrowserExtensionButton = document.querySelector("#setupBrowserExtensionButton");
  ...
  async runFirstRunProviderSetupRecovery({
    ...
    return {
      ok: Boolean(providerItem)
        ...
        && !bodyText.includes("placeholder-gui-smoke-secret"),
...
async function refreshOfficeAddinSetupStatus() {
  ...
  try {
    const status = await getOfficeAddinsSetupStatusViaShell();
...
function renderBrowserExtensionSetupStatus(status) {
  if (!browserExtensionSetupState) {
    return;
  }

  if (!status) {
    browserExtensionSetupState.textContent = "Not checked yet.";
    return;
  }

  const readyText = status.ok ? "Ready" : "Needs setup";
  const hostReady = status.nativeHost?.exeExists ? "host: ready" : "host: missing";
  const manifestText = status.extensionManifestExists ? "extension: ready" : "extension: missing";
  const chromeText = status.chrome?.selected ? `Chrome: ${status.chrome.registered ? "registered" : "pending"}` : "";
  const edgeText = status.edge?.selected ? `Edge: ${status.edge.registered ? "registered" : "pending"}` : "";
  const idText = status.stableExtensionId ? `ID: ${status.stableExtensionId}` : "";
  const folderText = status.extensionDir ? `folder: ${status.extensionDir}` : "";
  browserExtensionSetupState.textContent = [
    readyText,
    manifestText,
    hostReady,
    chromeText,
  ...
  browserExtensionSetupState.className = status.ok ? "muted ready-text" : "muted";
}

async function refreshBrowserExtensionSetupStatus() {
  if (!browserExtensionSetupState) {
    return;
  }
  browserExtensionSetupState.textContent = "Checking browser extension setup...";
  try {
    const status = await getBrowserExtensionSetupStatusViaShell();
    renderBrowserExtensionSetupStatus(status);
  } catch (error) {
    browserExtensionSetupState.textContent = `Check failed: ${error.message}`;
  }
}

async function configureBrowserExtension() {
  if (!browserExtensionSetupState) {
    return;
  }
  setupBrowserExtensionButton.disabled = true;
  browserExtensionSetupState.textContent = "Configuring... Browser extensions page and folder may open.";
  try {
    const status = await setupBrowserExtensionViaShell({
      browser: "both",
      openExtensionPage: true,
      openExtensionFolder: true
    });
    renderBrowserExtensionSetupStatus(status);
  } catch (error) {
    browserExtensionSetupState.textContent = `Configure failed: ${error.message}`;
  } finally {
    setupBrowserExtensionButton.disabled = false;
  }
}
...
async function getOfficeAddinsSetupStatusViaShell() {
  if (typeof consoleShellClient?.getOfficeAddinsSetupStatus !== "function") {
    throw new Error("Desktop Office add-in status bridge unavailable.");
  }
  const result = await consoleShellClient.getOfficeAddinsSetupStatus();
  if (result?.ok === false && result?.error) {
    throw new Error(result.message ?? result.error ?? "Could not check Office add-ins.");
  }
  return result ?? {};
}

async function setupBrowserExtensionViaShell(payload) {
  if (typeof consoleShellClient?.setupBrowserExtension !== "function") {
    throw new Error("Desktop browser extension setup bridge unavailable.");
  }
  const result = await consoleShellClient.setupBrowserExtension(payload ?? {});
  if (result?.ok === false && result?.error) {
    throw new Error(result.message ?? result.error ?? "Could not configure browser extension.");
  }
  return result ?? {};
}

async function getBrowserExtensionSetupStatusViaShell() {
  if (typeof consoleShellClient?.getBrowserExtensionSetupStatus !== "function") {
    throw new Error("Desktop browser extension status bridge unavailable.");
  }
  const result = await consoleShellClient.getBrowserExtensionSetupStatus();
  if (result?.ok === false && result?.error) {
    throw new Error(result.message ?? result.error ?? "Could not check browser extension.");
  }
  return result ?? {};
}
...
checkBrowserExtensionButton?.addEventListener("click", () => {
  void refreshBrowserExtensionSetupStatus();
});

setupBrowserExtensionButton?.addEventListener("click", () => {
  void configureBrowserExtension();
});
