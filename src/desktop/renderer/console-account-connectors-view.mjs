...
export const ACCOUNT_CONNECTOR_META = Object.freeze({
  ...
  google: Object.freeze({
    ...
    setupTitle: "使用内置 OAuth 应用，或填自己的 Client ID",
    setupSteps: Object.freeze([
      "发布版可内置 LingxY 的 public OAuth Client ID，用户直接点授权登录",
      "如果要用自己的 Google 项目，打开 Google Cloud Console → API 和服务 → 凭据",
      "创建凭据 → OAuth 客户端 ID → 类型选\"桌面应用\"",
      "复制客户端 ID 粘贴到下方；桌面 PKCE 流不要依赖客户端密码",
      "公开给其他用户前，OAuth 同意屏幕需要发布/验证；测试模式只允许测试用户"
    ]),
...
export function renderAccountConnectorConfigPanelHtml(type, cfgData = {}) {
  ...
  const usesDefault = cfgData.usingDefaultClientId === true;
  const defaultCopy = usesDefault
    ? `<p style="font-size:11px;color:var(--muted);margin:0;">正在使用发布版内置 OAuth 应用。这里留空即可；填写自己的 Client ID 会覆盖默认值。</p>`
    : "";
  ...
  return `
    <div class="acc-config-panel" style="padding:12px 14px;background:var(--panel-2);border:1px solid var(--line);border-radius:var(--radius-sm);margin-top:-2px;display:flex;flex-direction:column;gap:10px;">
      ...
      </div>` : `<p style="font-size:11px;color:var(--muted);margin:0;">✓ ${escapeHtml(meta.label)} 桌面 PKCE 流无需 Client Secret</p>`}
