...
[
  ...
  ["Schedule", "定时"],
  ["Voice note", "录音笔记"],
  ["Type what you need...", "输入你的需求..."],
  ["Start new session", "开始新会话"],
  ["Start new session (clear memory)", "开始新会话（清空记忆）"],
  ["Translate current context", "翻译当前上下文"],
  ["Summarize current context", "总结当前上下文"],
  ["Explain current context", "解释当前上下文"],
  ["Create scheduled task", "新建定时任务"],
  ["Open voice note", "打开录音笔记"],
  ["Open settings", "打开设置"],
  ["Send instruction", "发送指令"],
  ["Read clipboard", "读取剪贴板"],
  ["Read clipboard as context", "读取剪贴板内容作为上下文"],
  ["Command input (Enter to send, Shift+Enter for newline)", "输入指令（按 Enter 发送，Shift+Enter 换行）"],
  ["Command input", "指令输入栏"],
  ...
  ["Switch projects / view conversation history", "切换项目 / 查看历史会话"],
  ["Switch projects or view conversation history", "切换项目或查看历史会话"],
