...
let pendingActiveWindowActionContext = null;
let pendingActiveWindowActionContextCapturedAt = 0;
let shortcutCaptureSession = null;
let acceptedHotkeyCaptureSessionId = "";
let acceptedHotkeyCaptureSessionExpiresAt = 0;
const SHORTCUT_CAPTURE_WATCHDOG_MS = 2600;
const SCREENSHOT_CAPTURE_WATCHDOG_MS = 10000;
const EXPLICIT_BROWSER_CONTEXT_FALLBACK_MAX_AGE_MS = 30 * 1000;
const EXPLICIT_CURRENT_CONTEXT_CAPTURE_TIMEOUT_MS = 1600;
const BROWSER_TEXT_CONTEXT_WAIT_MS = 1800;
const BROWSER_RECENT_TEXT_FALLBACK_MAX_AGE_MS = 45 * 1000;
const BROWSER_EXPLICIT_PAGE_TEXT_FALLBACK_MAX_AGE_MS = 5 * 60 * 1000;
...
function startNewConversation({ preservePendingInputContext = false } = {}) {
  ...
  if (preservePendingInputContext) {
    ...
    pendingActiveWindowActionContext = null;
    pendingActiveWindowActionContextCapturedAt = 0;
...
function rememberAcceptedHotkeyCaptureSession(captureSessionId = "") {
  const id = `${captureSessionId ?? ""}`.trim();
  if (!id) return;
  acceptedHotkeyCaptureSessionId = id;
  acceptedHotkeyCaptureSessionExpiresAt = Date.now() + Math.max(5000, SHORTCUT_CAPTURE_WATCHDOG_MS * 2);
}

function isRecentlyAcceptedHotkeyCaptureSession(captureSessionId = "") {
  const id = `${captureSessionId ?? ""}`.trim();
  if (!id || id !== acceptedHotkeyCaptureSessionId) return false;
  return Date.now() <= acceptedHotkeyCaptureSessionExpiresAt;
}
...
function captureSessionIdFromPayload(payload = {}) {
  return `${payload?.capture_session_id ?? payload?.captureSessionId ?? ""}`.trim();
}

function captureStartedAtFromPayload(payload = {}) {
  const value = Number(payload?.capture_started_at ?? payload?.captureStartedAt ?? 0);
...
function beginShortcutCaptureSession(shortcutId = "", {
  ...
  if (!captureSessionId && Date.now() - lastShellHandoffAt < 800) return;
  clearShortcutCaptureSession();
  ...
  shortcutCaptureSession = {
    shortcutId,
    captureSessionId,
    captureStartedAt: captureStartedAt || Date.now(),
    bubble,
    timer: setTimeout(() => {
...
function setSystemBubbleText(bubble, text) {
  if (!bubble?.isConnected) return false;
...
function removeBubbleElement(bubble) {
  if (!bubble?.isConnected) return false;
  bubble.remove();
  if (bubbleArea && bubbleArea.children.length === 0) {
    bubbleArea.hidden = true;
...
async function submitTask() {
  ...
  let currentContextStatusBubble = null;
  const showCurrentContextStatus = (text) => {
    currentContextStatusBubble = addSystemBubble(text);
    currentContextStatusBubble.dataset.ucaCurrentContextCaptureStatus = "1";
  };
  const finishCurrentContextStatus = (text) => {
    if (!setSystemBubbleText(currentContextStatusBubble, text)) {
      addSystemBubble(text);
    }
  };
  const clearCurrentContextStatus = () => {
    removeBubbleElement(currentContextStatusBubble);
    currentContextStatusBubble = null;
  };
  ...
  try {
    ...
    const explicitWindowContextRequest = commandTargetsCurrentWindowContext(commandText);
    const explicitActiveWindowHint = explicitBrowserContextRequest || explicitFileContextRequest || explicitWindowContextRequest
      ? freshPendingActiveWindowActionContext()
      : null;
    if (explicitBrowserContextRequest) {
      showCurrentContextStatus("正在捕捉当前页面…");
    } else if (explicitFileContextRequest) {
      showCurrentContextStatus("正在捕捉当前文件…");
    } else if (explicitWindowContextRequest) {
      showCurrentContextStatus("正在捕捉当前窗口…");
    }
    let activeBrowserCapture = explicitBrowserContextRequest || explicitWindowContextRequest
      ? await resolveActiveWindowBrowserCapture({
          preferredActiveWindow: explicitActiveWindowHint,
          allowProbe: !explicitActiveWindowHint,
          allowRecentFallback: Boolean(explicitActiveWindowHint && isBrowserProcessWindow(explicitActiveWindowHint))
        })
      : null;
    const activeFileSelection = !activeBrowserCapture
      && (explicitFileContextRequest || explicitBrowserContextRequest || explicitWindowContextRequest)
      ? await resolveActiveWindowFileSelection(commandText, {
          preferredActiveWindow: explicitActiveWindowHint,
          allowProbe: !explicitActiveWindowHint,
          allowBrowserFileUrlForPage: explicitBrowserContextRequest || explicitWindowContextRequest,
          allowCurrentWindowFile: explicitWindowContextRequest
        })
      : null;
    const activeWindowTextCapture = !activeBrowserCapture
      && !activeFileSelection?.filePaths?.length
      && (explicitFileContextRequest || explicitWindowContextRequest)
      ? await resolveActiveWindowTextCapture({
          preferredActiveWindow: explicitActiveWindowHint,
          allowProbe: !explicitActiveWindowHint
        })
      : null;
    if (!activeBrowserCapture
      ...
      activeBrowserCapture = await resolveExplicitBrowserTextFallback({
        preferredActiveWindow: explicitActiveWindowHint
      });
    }
    const allowBrowserScreenshotFallback = Boolean(explicitActiveWindowHint && isBrowserProcessWindow(explicitActiveWindowHint));
    const activeWindowScreenshotSelection = !activeBrowserCapture
      && !activeFileSelection?.filePaths?.length
      && !activeWindowTextCapture
      && (explicitWindowContextRequest
        || explicitFileContextRequest
        || (explicitBrowserContextRequest && allowBrowserScreenshotFallback))
      ? await resolveActiveWindowScreenshotSelection({
          preferredActiveWindow: explicitActiveWindowHint,
          allowProbe: explicitWindowContextRequest && !explicitActiveWindowHint
        })
      : null;
    if (explicitBrowserContextRequest
        ...
        && !activeFileSelection?.filePaths?.length
        && !activeWindowTextCapture
        && !activeWindowScreenshotSelection?.filePaths?.length) {
      commandInput.value = rawCommand;
      autoSizeInput();
      finishCurrentContextStatus("没有检测到可读取的当前浏览器页面。请先切到要分析的网页，或选中文字/链接后再提问。");
      commandInput.focus();
      return;
    }
    if (explicitWindowContextRequest
      ...
      commandInput.value = rawCommand;
      autoSizeInput();
      finishCurrentContextStatus("没有检测到可读取的当前窗口内容。请先切到网页、文件或普通应用窗口，或选中内容后再提问。");
      commandInput.focus();
      return;
    }
    const canUseAmbientActiveWindow = !explicitBrowserContextRequest
      && !explicitFileContextRequest
      && !explicitWindowContextRequest
      && !pendingFileSelection?.filePaths?.length
      && !pendingCapture?.capture;
    const ambientActiveWindow = canUseAmbientActiveWindow
      ? await resolvePendingActiveWindowResourceCapture({
          allowProbe: true,
          allowScreenshotFallback: true
        })
      : { activeWindowBrowserCapture: null, activeWindowFileSelection: null, activeWindowCapture: null };
    ...
    const contextDecision = resolveOverlayContextSubmission({
      ...
      explicitWindowContextRequest,
      explicitFileContextRequest,
      activeFileSelection: activeFileSelection ?? activeWindowScreenshotSelection,
      activeWindowBrowserCapture: ambientActiveWindow.activeWindowBrowserCapture,
      activeWindowFileSelection: ambientActiveWindow.activeWindowFileSelection,
      activeWindowCapture: activeWindowTextCapture ?? ambientActiveWindow.activeWindowCapture,
    ...
    if (contextDecision.kind === "capture") {
      ...
      commandInput.value = rawCommand;
      autoSizeInput();
      finishCurrentContextStatus("没有检测到可读取的当前文件。请先选中文件、拖入文件，或把文件窗口切到前台后再提问。");
      commandInput.focus();
      return;
      ...
      payload = {
        ...
        captureMode: contextDecision.captureMode ?? "shell_menu",
        selectionMetadata: contextDecision.selectionMetadata ?? {},
        executionMode: "interactive"
      };
...
function rememberPendingActiveWindowActionContext(activeWindow = null) {
  if (!activeWindow || activeWindow.blocked) return;
  rememberPendingActiveWindowContext(activeWindow);
  pendingActiveWindowActionContext = { ...activeWindow };
  pendingActiveWindowActionContextCapturedAt = Date.now();
}

function freshPendingActiveWindowActionContext(maxAgeMs = EXPLICIT_BROWSER_CONTEXT_FALLBACK_MAX_AGE_MS) {
  if (!pendingActiveWindowActionContext) return null;
  if (!pendingActiveWindowActionContextCapturedAt) return null;
  if (Date.now() - pendingActiveWindowActionContextCapturedAt > maxAgeMs) return null;
  return pendingActiveWindowActionContext;
}
...
function showActiveWindowPreviewCard(activeWindow) {
  ...
  let subLabel = "";
  let quickActions = [];
  const fileInfo = activeWindowFilePathInfo(activeWindow);

  if (kind === "web_url" && isHttpUrl(activeWindow.url)) {
    icon = "🌐";
  ...
  } else if (isBrowserProcessWindow(activeWindow) && title) {
    icon = "🌐";
    label = `当前浏览器：${title}`;
    subLabel = "可以尝试读取浏览器扩展最近捕捉的页面内容";
    quickActions = [
      { label: "分析此页面", command: "分析当前页面的完整内容并总结要点" },
      { label: "提取关键信息", command: "从当前页面里抽取最重要的数据点" }
    ];
  ...
  addBubble("assistant", card, {
    optionButtons: quickActions.map((action) => ({
      ...
      onClick: () => {
        startNewConversation();
        rememberPendingActiveWindowActionContext(activeWindow);
...
function isActiveBrowserWindow(activeWindow = null) {
  const kind = activeWindow?.detected_kind ?? activeWindow?.detectedKind;
  if (kind !== "web_url") return false;
  return isHttpUrl(activeWindow?.url);
}
...
function isBrowserProcessWindow(activeWindow = null) {
  const process = `${activeWindow?.process ?? ""}`.trim().toLowerCase().replace(/\.exe$/i, "");
  if (!process) return false;
  return BROWSER_PROCESS_NAMES.has(process);
}
...
function activeWindowFilePathInfo(activeWindow = null) {
  const kind = activeWindow?.detected_kind ?? activeWindow?.detectedKind;
  if (kind === "file_path") {
    const filePath = `${activeWindow?.file_path ?? activeWindow?.filePath ?? ""}`.trim();
    return filePath ? { filePath, sourceKind: "probe_file_path" } : null;
  }
  if (kind === "web_url") {
    const filePath = filePathFromFileUrl(activeWindow?.url ?? "");
    return filePath ? { filePath, sourceKind: "browser_file_url" } : null;
  }
  return null;
}

function activeWindowFilePath(activeWindow = null) {
  return activeWindowFilePathInfo(activeWindow)?.filePath ?? "";
}

function activeWindowText(activeWindow = null) {
  const text = `${activeWindow?.text ?? activeWindow?.documentText ?? ""}`.trim();
  return text.length > 0 ? text : "";
}

function activeWindowContextMetadata(activeWindow = null, {
  extractionMode = "active_window"
} = {}) {
  if (!activeWindow || activeWindow.blocked) return {};
  return {
    current_window_context: true,
    active_window_context: true,
    active_window: {
      process: activeWindow.process ?? null,
      title: activeWindow.title ?? null,
      detected_kind: activeWindow.detected_kind ?? activeWindow.detectedKind ?? null,
      url: activeWindow.url ?? null,
      file_path: activeWindow.file_path ?? activeWindow.filePath ?? null,
      bounds: activeWindow.bounds ?? null,
      extra: activeWindow.extra ?? {}
    },
    content_evidence: [{
      schema_version: "1.0",
      source_kind: "active_window",
      coverage_scope: extractionMode === "active_window_screenshot"
        ? "visible_window_pixels"
        : (extractionMode === "active_window_text" ? "active_window_text" : "active_window_resource"),
      content_extracted: extractionMode !== "active_window_title",
      status: "available",
      modality: extractionMode === "active_window_screenshot" ? "image" : "text",
      locator: activeWindow.file_path ?? activeWindow.filePath ?? activeWindow.url ?? null,
      title: activeWindow.title ?? null,
      extraction_mode: extractionMode,
      pixels_available: extractionMode === "active_window_screenshot"
    }]
  };
}

function buildActiveWindowTextCapture(activeWindow = null) {
  const text = activeWindowText(activeWindow);
  if (!text) return null;
  const extra = activeWindow?.extra && typeof activeWindow.extra === "object"
    ? activeWindow.extra
    : {};
  return {
    sourceType: "text",
    browser: activeWindow?.process ?? "active-window",
    pageTitle: activeWindow?.title ?? "",
    text,
    selectionMetadata: {
      ...activeWindowContextMetadata(activeWindow, {
        extractionMode: "active_window_text"
      }),
      office_app: extra.office_app ?? null,
      active_window_text_context: true
    },
    metadata: {
      contentKind: "active_window_text",
      activeWindowContext: true,
      officeApp: extra.office_app ?? null,
      documentName: extra.document_name ?? null
    }
  };
}
...
function browserContextHasUsefulText(browserContext = null) {
  if (!browserContext) return false;
  const text = `${browserContext.text ?? ""}`.replace(/\s+/g, " ").trim();
  const youtube = browserContext?.metadata?.youtube ?? null;
  const description = `${browserContext?.metadata?.description ?? youtube?.description ?? ""}`.trim();
  const captions = `${youtube?.visibleCaptions ?? ""}`.trim();
  return Boolean(text || description || captions);
}

async function fetchRecentBrowserContextForActiveWindow(activeWindow = null, {
  allowRecentFallback = false,
  requireText = false,
  maxAgeMs = BROWSER_RECENT_TEXT_FALLBACK_MAX_AGE_MS,
  minTextLength = 80,
  timeoutMs = 900
} = {}) {
  const hasExactUrl = isActiveBrowserWindow(activeWindow);
  if (!hasExactUrl && !isBrowserProcessWindow(activeWindow)) return null;
  const title = `${activeWindow?.title ?? ""}`.trim();
  if (!hasExactUrl && !title && !allowRecentFallback) return null;
  ...
  if (allowRecentFallback) {
    params.set("allow_recent_fallback", "1");
    params.set("max_age_ms", String(maxAgeMs));
  }
  if (requireText) {
    params.set("require_text", "1");
    params.set("min_text_length", String(minTextLength));
  }
  params.set("limit", "1");
...
async function fetchLatestRecentBrowserTextContext({
  maxAgeMs = BROWSER_EXPLICIT_PAGE_TEXT_FALLBACK_MAX_AGE_MS,
  minTextLength = 80,
  timeoutMs = 900
} = {}) {
  const params = new URLSearchParams();
...
function buildBrowserContextCapture(browserContext = null, activeWindow = null) {
  ...
  if (pageText) sections.push(`## 页面可见文本\n${compactBrowserContextText(pageText, 18000)}`);
  ...
  return {
    ...
    selectionMetadata: {
      active_window_context: true,
      browser_context_source: browserContext ? "extension_recent_snapshot" : "active_window_url",
      browser_context_match_reason: browserContext?.matchReason ?? null
    },
    metadata: {
      contentKind: youtube ? "video" : "webpage",
      platform: platform || "generic",
      browserContextScore: browserContext?.score ?? null,
      browserContextMatchReason: browserContext?.matchReason ?? null,
...
async function waitForRecentBrowserContext(activeWindow = null, options = {}) {
  const waitMs = Math.max(0, Number(options.waitMs ?? 0) || 0);
  const startedAt = Date.now();
  let lastContext = null;
  while (Date.now() - startedAt <= waitMs) {
    lastContext = await fetchRecentBrowserContextForActiveWindow(activeWindow, options);
    if (browserContextHasUsefulText(lastContext)) return lastContext;
    if (waitMs === 0) break;
...
async function resolveBrowserCaptureFromActiveWindow(activeWindow = null, {
  allowRecentFallback = false,
  waitForTextMs = 0
} = {}) {
  if (!isActiveBrowserWindow(activeWindow) && !isBrowserProcessWindow(activeWindow)) return null;
  const browserContext = await waitForRecentBrowserContext(activeWindow, {
    allowRecentFallback,
    requireText: allowRecentFallback,
    waitMs: waitForTextMs,
    timeoutMs: 900,
    maxAgeMs: BROWSER_RECENT_TEXT_FALLBACK_MAX_AGE_MS,
    minTextLength: 80
  });
  return buildBrowserContextCapture(browserContext, activeWindow);
}

async function resolveActiveWindowBrowserCapture({
  preferredActiveWindow = null,
  allowProbe = true,
  allowRecentFallback = false
} = {}) {
  const preferredCapture = await resolveBrowserCaptureFromActiveWindow(preferredActiveWindow, {
    allowRecentFallback,
    waitForTextMs: allowRecentFallback ? BROWSER_TEXT_CONTEXT_WAIT_MS : 0
  });
  if (preferredCapture) {
    rememberPendingActiveWindowContext(preferredActiveWindow);
    return preferredCapture;
  }

  let activeWindow = null;
  if (allowProbe && typeof overlayShellClient?.getActiveWindowContext === "function") {
    const payload = await timeoutWithFallback(
      overlayShellClient.getActiveWindowContext({
        ...
        timeoutMs: EXPLICIT_CURRENT_CONTEXT_CAPTURE_TIMEOUT_MS
      }),
      EXPLICIT_CURRENT_CONTEXT_CAPTURE_TIMEOUT_MS + 200,
      null
    );
    activeWindow = payload?.active_window ?? payload?.activeWindow ?? null;
    if (isActiveBrowserWindow(activeWindow) || isBrowserProcessWindow(activeWindow)) {
      rememberPendingActiveWindowContext(activeWindow);
    }
  }

  const freshActiveWindow = freshPendingActiveWindowContext(EXPLICIT_BROWSER_CONTEXT_FALLBACK_MAX_AGE_MS);
  const candidate = isActiveBrowserWindow(activeWindow) || isBrowserProcessWindow(activeWindow)
    ? activeWindow
    : (isActiveBrowserWindow(freshActiveWindow) || isBrowserProcessWindow(freshActiveWindow) ? freshActiveWindow : null);
  if (!candidate) return null;
  return resolveBrowserCaptureFromActiveWindow(candidate, {
    allowRecentFallback,
    waitForTextMs: allowRecentFallback ? BROWSER_TEXT_CONTEXT_WAIT_MS : 0
  });
}

async function resolveExplicitBrowserTextFallback({
  preferredActiveWindow = null
} = {}) {
  const candidates = [];
  const seen = new Set();
  const addCandidate = (activeWindow) => {
    if (!isActiveBrowserWindow(activeWindow) && !isBrowserProcessWindow(activeWindow)) return;
    const key = [
      browserProcessName(activeWindow),
      `${activeWindow?.url ?? ""}`.trim(),
      `${activeWindow?.title ?? ""}`.trim()
    ].join("\n");
    if (seen.has(key)) return;
    seen.add(key);
    candidates.push(activeWindow);
  };

  addCandidate(preferredActiveWindow);
  addCandidate(freshPendingActiveWindowActionContext(BROWSER_EXPLICIT_PAGE_TEXT_FALLBACK_MAX_AGE_MS));
  addCandidate(freshPendingActiveWindowContext(BROWSER_EXPLICIT_PAGE_TEXT_FALLBACK_MAX_AGE_MS));

  for (const candidate of candidates) {
    const browserContext = await waitForRecentBrowserContext(candidate, {
      allowRecentFallback: true,
      requireText: true,
      waitMs: BROWSER_TEXT_CONTEXT_WAIT_MS,
      timeoutMs: 900,
      maxAgeMs: BROWSER_EXPLICIT_PAGE_TEXT_FALLBACK_MAX_AGE_MS,
      minTextLength: 80
    });
    if (browserContextHasUsefulText(browserContext)) {
      return buildBrowserContextCapture(browserContext, candidate);
    }
  }

  const browserContext = await fetchLatestRecentBrowserTextContext({
    maxAgeMs: BROWSER_EXPLICIT_PAGE_TEXT_FALLBACK_MAX_AGE_MS,
    minTextLength: 80,
    timeoutMs: 900
  });
  if (!browserContextHasUsefulText(browserContext)) return null;
  return buildBrowserContextCapture(browserContext, {
    process: browserContext.browser || "chrome.exe",
    title: browserContext.pageTitle || "",
    url: browserContext.url || "",
    detected_kind: browserContext.url ? "web_url" : "browser_recent_text"
  });
}

async function captureExternalActiveWindowForSubmit({
  captureMode = "current_context_submit",
  timeoutMs = EXPLICIT_CURRENT_CONTEXT_CAPTURE_TIMEOUT_MS
} = {}) {
  ...
  try {
    ...
    if (activeWindowHasUsableIdentity(activeWindow)
        || isActiveBrowserWindow(activeWindow)
        || isBrowserProcessWindow(activeWindow)
        || activeWindowFilePath(activeWindow)) {
      rememberPendingActiveWindowContext(activeWindow);
    }
    return activeWindow;
  } catch {
...
async function resolveActiveWindowFileSelection(commandText = "", {
  preferredActiveWindow = null,
  allowProbe = true,
  allowBrowserFileUrlForPage = false,
  allowCurrentWindowFile = false
} = {}) {
  const hintedActiveWindow = activeWindowFilePath(preferredActiveWindow) ? preferredActiveWindow : null;
  const probedActiveWindow = hintedActiveWindow || !allowProbe
    ? null
    : await captureExternalActiveWindowForSubmit({
        captureMode: allowBrowserFileUrlForPage ? "current_page_file_submit" : "current_file_submit",
        timeoutMs: EXPLICIT_CURRENT_CONTEXT_CAPTURE_TIMEOUT_MS
      });
  const candidate = activeWindowFilePath(probedActiveWindow)
    ? probedActiveWindow
    : (hintedActiveWindow ?? freshPendingActiveWindowContext());
  const fileInfo = activeWindowFilePathInfo(candidate);
  if (!fileInfo?.filePath) return null;
  const explicitFileIntent = commandTargetsCurrentFileContext(commandText);
  const allowedFromBrowserFileUrl = allowBrowserFileUrlForPage && fileInfo.sourceKind === "browser_file_url";
  if (!explicitFileIntent && !allowedFromBrowserFileUrl && !allowCurrentWindowFile) return null;
  return {
    sourceApp: candidate?.process ?? "active-window",
    captureMode: fileInfo.sourceKind === "browser_file_url" ? "active_window_file_url" : "active_window_file",
    sourceKind: fileInfo.sourceKind,
    selectionMetadata: activeWindowContextMetadata(candidate, {
      extractionMode: fileInfo.sourceKind === "browser_file_url" ? "active_window_file_url" : "active_window_file"
    }),
    filePaths: [fileInfo.filePath]
  };
}

async function resolveActiveWindowTextCapture({
  preferredActiveWindow = null,
  allowProbe = true
} = {}) {
  const hintedCapture = buildActiveWindowTextCapture(preferredActiveWindow);
  if (hintedCapture) return hintedCapture;

  const probedActiveWindow = allowProbe
    ? await captureExternalActiveWindowForSubmit({
        captureMode: "current_window_text_submit",
        timeoutMs: EXPLICIT_CURRENT_CONTEXT_CAPTURE_TIMEOUT_MS
      })
    : null;
  const probedCapture = buildActiveWindowTextCapture(probedActiveWindow);
  if (probedCapture) return probedCapture;

  return buildActiveWindowTextCapture(
    freshPendingActiveWindowContext(EXPLICIT_BROWSER_CONTEXT_FALLBACK_MAX_AGE_MS)
  );
}

function activeWindowHasUsableIdentity(activeWindow = null) {
  if (!activeWindow || activeWindow.blocked) return false;
  return Boolean(`${activeWindow.process ?? ""}`.trim() || `${activeWindow.title ?? ""}`.trim());
}

async function resolveActiveWindowScreenshotSelection({
  preferredActiveWindow = null,
  allowProbe = true
} = {}) {
  if (typeof overlayShellClient?.captureWindowScreenshot !== "function") return null;
  const activeWindow = allowProbe
    ? await captureExternalActiveWindowForSubmit({
        captureMode: "current_window_screenshot_probe",
        timeoutMs: Math.max(EXPLICIT_CURRENT_CONTEXT_CAPTURE_TIMEOUT_MS, 2200)
      })
    : null;
  const candidate = activeWindowHasUsableIdentity(activeWindow)
    ? activeWindow
    : (activeWindowHasUsableIdentity(preferredActiveWindow)
        ? preferredActiveWindow
        : freshPendingActiveWindowContext(EXPLICIT_BROWSER_CONTEXT_FALLBACK_MAX_AGE_MS));
  if (!activeWindowHasUsableIdentity(candidate)) return null;
  try {
    const screenshot = await timeoutWithFallback(
      overlayShellClient.captureWindowScreenshot({
        excludeShellWindow: true,
    ...
    if (!screenshot?.ok || !screenshot.path) return null;
    return {
      sourceApp: candidate?.process ?? "active-window",
      captureMode: "active_window_screenshot",
      sourceKind: "active_window_screenshot",
      selectionMetadata: activeWindowContextMetadata(candidate, {
        extractionMode: "active_window_screenshot"
      }),
      filePaths: [screenshot.path]
    };
  } catch {
    return null;
  }
...
async function resolvePendingActiveWindowResourceCapture({
  allowProbe = false,
  allowScreenshotFallback = false
} = {}) {
  let activeWindow = freshPendingActiveWindowContext();
  if (!activeWindow && allowProbe) {
    activeWindow = await captureExternalActiveWindowForSubmit({
      captureMode: "ambient_current_window_submit",
      timeoutMs: EXPLICIT_CURRENT_CONTEXT_CAPTURE_TIMEOUT_MS
    });
  }
  if (isActiveBrowserWindow(activeWindow) || isBrowserProcessWindow(activeWindow)) {
    const browserContext = await fetchRecentBrowserContextForActiveWindow(activeWindow);
    const capture = buildBrowserContextCapture(browserContext, activeWindow);
    if (capture) {
      return {
        activeWindowBrowserCapture: capture,
  ...
  if (fileInfo?.filePath) {
    return {
      ...
      activeWindowFileSelection: {
        sourceApp: activeWindow?.process ?? "active-window",
        captureMode: fileInfo.sourceKind === "browser_file_url" ? "active_window_file_url" : "active_window_file",
        sourceKind: fileInfo.sourceKind,
        selectionMetadata: activeWindowContextMetadata(activeWindow, {
          extractionMode: fileInfo.sourceKind === "browser_file_url" ? "active_window_file_url" : "active_window_file"
        }),
        filePaths: [fileInfo.filePath]
      }
  ...
  const activeWindowCapture = buildActiveWindowTextCapture(activeWindow);
  ...
  if (allowScreenshotFallback && activeWindowHasUsableIdentity(activeWindow)) {
    const screenshotSelection = await resolveActiveWindowScreenshotSelection({
      preferredActiveWindow: activeWindow,
      allowProbe: false
    });
    if (screenshotSelection?.filePaths?.length) {
      return {
        activeWindowBrowserCapture: null,
        activeWindowFileSelection: screenshotSelection,
...
function applyShellHandoff(payload) {
  ...
  const incomingCaptureSessionId = captureSessionIdFromPayload(payload);
  if (isHotkeyCapture && incomingCaptureSessionId) {
    const currentCaptureSessionId = shortcutCaptureSession?.captureSessionId ?? "";
    if (currentCaptureSessionId !== incomingCaptureSessionId
        && !isRecentlyAcceptedHotkeyCaptureSession(incomingCaptureSessionId)) {
      return;
    }
    rememberAcceptedHotkeyCaptureSession(incomingCaptureSessionId);
  }
  ...
  if (payload?.file_paths?.length) {
    ...
    pendingFileSelection = {
      ...
      selectionMetadata: {
        ...
        capture_session_id: incomingCaptureSessionId || null,
        capture_started_at: captureStartedAtFromPayload(payload) || null,
        capture_received_at: Date.now()
      },
...
overlayShellClient.onShortcutTriggered((payload) => {
  ...
  if (payload.shortcutId === "capture-and-ask") {
    beginShortcutCaptureSession(payload.shortcutId, {
      captureSessionId: captureSessionIdFromPayload(payload),
      captureStartedAt: captureStartedAtFromPayload(payload)
    });
  }
